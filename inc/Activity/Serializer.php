...
final class Serializer {
	...
	public static function normalize_entry( array $entry ): array {
		...
		return [
			...
			'applyLane'             => in_array( $entry['applyLane'] ?? null, [ 'editor-state', 'server-executed' ], true ) ? $entry['applyLane'] : null,
			'linkedApplyActivityId' => self::normalize_nullable_string( $entry['linkedApplyActivityId'] ?? null ),
			'saveOccurrenceId'      => self::normalize_nullable_string( $entry['saveOccurrenceId'] ?? null ),
	...
	public static function hydrate_row( array $row ): array {
		...
		$hydrated = [
			...
			'applyLane'             => self::normalize_nullable_string( $row['apply_lane'] ?? null ),
			'linkedApplyActivityId' => self::normalize_nullable_string( $row['linked_apply_activity_id'] ?? null ),
			'saveOccurrenceId'      => self::normalize_nullable_string( $row['save_occurrence_id'] ?? null ),
			'undo'                  => $undo,
			'userId'                => $user_id,
