...
function getBindableAttributes(blockName) {
	...
	const supportedAttributes =
		settings?.__experimentalBlockBindingsSupportedAttributes?.[blockName];

	if (settings?.canUpdateBlockBindings !== true || !Array.isArray(supportedAttributes)) {
		return [];
	}

	return [...new Set(
		supportedAttributes
			.filter((attribute) => typeof attribute === 'string' && attribute.trim() !== '')
			.map((attribute) => attribute.trim())
	)];
}

function addBindingsPanel(blockName, inspectorPanels) {
	const bindableAttributes = getBindableAttributes(blockName);

	if (!bindableAttributes.length) {
		return {
			bindableAttributes,
			inspectorPanels,
		};
...
export function introspectBlockType(blockName) {
	...
	const {
		bindableAttributes,
		inspectorPanels,
	} = addBindingsPanel(blockName, resolveInspectorPanels(supports));
