...
function scanForTokens(
	...
	for ( const filePath of files ) {
		...
		if ( matches.size === loweredTokens.length ) {
			break;
		}
	}
...
function findFilesRecursive(
	...
	while ( queue.length > 0 ) {
		...
		for ( const ent of entries ) {
			...
			if ( ent.isDirectory() ) {
				if ( DEFAULT_IGNORES.has( ent.name ) ) {
					continue;
				}
...
function guessWpCoreVersionFromCheckout( repoRoot ) {
	...
	if ( ! match ) {
		return { value: null, source: 'wp-includes/version.php' };
	}
...
function detectKinds( repoRoot, signals ) {
	...
	if ( signals.isGutenbergRepo ) {
		kinds.add( 'gutenberg' );
	}
	if ( signals.isWpCoreCheckout ) {
		kinds.add( 'wp-core' );
	}
	if ( signals.hasWpContentDir ) {
		kinds.add( 'wp-site' );
	}
	...
	if ( signals.hasMuPluginsDir ) {
		kinds.add( 'wp-mu-plugin' );
	}
