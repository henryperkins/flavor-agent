...
- `OpenAI\Provider`, `AzureOpenAI\` (shared embedding clients for OpenAI Native / Workers AI / legacy Azure, Qdrant vector DB, embedding signature for cache invalidation, shared HTTP base, `ResponsesClient` compat facade), `Cloudflare\` (AI Search docs grounding, private pattern search, Workers AI embedding config — default model `@cf/qwen/qwen3-embedding-0.6b`) — embedding/provider plumbing.
...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Embeddings provider selection                | `flavor_agent_openai_provider` (`openai_native`, `cloudflare_workers_ai`, legacy `azure_openai`, or a connector ID that pins chat to that connector while embeddings fall back to a configured direct backend; Workers AI is never used as an implicit fallback)        |
| Chat (all surfaces)                          | Owned by core `Settings > Connectors` via the WordPress AI Client. No plugin-managed chat credentials.                                                                                                                                                                  |
| Legacy Azure OpenAI (embeddings)             | Previously saved `flavor_agent_azure_openai_endpoint`, `flavor_agent_azure_openai_key`, and `flavor_agent_azure_embedding_deployment` values are read only for older installs; the settings UI no longer renders or validates new Azure fields.                         |
