...
Flavor Agent is an AI-assisted editorial layer for WordPress 7.0+ that surfaces contextual recommendations across blocks, patterns, content, templates, navigation, Global Styles, and Style Book — with review-first apply, server-backed activity audit, and explicit ownership of every external service it can touch.
...
| Embeddings               | Plugin-owned OpenAI Native option for Qdrant           | Plugin-owned Azure embedding deployment for Qdrant | Not used for embeddings                 | Explicitly selected plugin-owned embedding backend for Qdrant | Not used for embeddings                             | Stores/searches vectors         |
...
| Configuration owner      | Connectors or `Settings > Flavor Agent` for embeddings | `Settings > Flavor Agent` for embeddings           | `Settings > Connectors`                 | `Settings > Flavor Agent`                                     | `Settings > Flavor Agent` or built-in docs endpoint | `Settings > Flavor Agent`       |
