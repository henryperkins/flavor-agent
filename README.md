...
**As of 2026-07-28 only the governance-console still exists.** Most remaining assets need a publicly reachable staged site with live provider credentials, so they cannot be captured from the local Docker stack. The one exception is `settings-readiness.png`, which needs the staged site and admin auth but no AI credentials — though a readiness panel showing everything unconfigured is weak proof, so a representative capture still wants at least one configured backend. Per-asset blockers are recorded in [`docs/releases/v0.1.0-proof-assets.md`](docs/releases/v0.1.0-proof-assets.md). Treat that as an open release gate, not a formality.
