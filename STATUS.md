...
- Site Editor style recommendation panels for the native Styles sidebar, with document-panel fallbacks, review-confirm-apply support for bounded `set_styles`, `set_block_styles`, and Global Styles-only `set_theme_variation` operations, shared capability/status messaging when the style backend is unavailable, and explicit `theme.json`-safe guardrails that keep raw CSS and `customCSS` out of scope
