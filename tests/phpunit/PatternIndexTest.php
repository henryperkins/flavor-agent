...
final class PatternIndexTest extends TestCase {
	...
	public function test_runtime_state_marks_workers_ai_model_change_as_embedding_signature_drift(): void {
		$this->register_pattern( 'theme/hero', $this->pattern_fixture( 'theme/hero', 'Hero', 'Hero copy' ) );
		$this->save_ready_state_for_patterns( $this->current_patterns() );

		WordPressTestState::$options['flavor_agent_cloudflare_workers_ai_embedding_model'] = '@cf/baai/bge-large-en-v1.5';
	...
	public function test_sync_refreshes_stale_compatible_state_without_signature_probe(): void {
		$this->register_pattern( 'theme/hero', $this->pattern_fixture( 'theme/hero', 'Hero', 'Hero copy' ) );
		$this->register_pattern( 'theme/footer-callout', $this->pattern_fixture( 'theme/footer-callout', 'Footer Callout', 'Footer copy' ) );
		...
		$this->save_ready_state_for_patterns(
			...
			[
				...
				'stale_reasons' => [ 'pattern_registry_changed' ],
			]
		);
	...
	public function test_sync_performs_full_reindex_when_embedding_endpoint_changes(): void {
		$this->register_pattern( 'theme/hero', $this->pattern_fixture( 'theme/hero', 'Hero', 'Hero copy' ) );
		...
		$this->assertSame( Provider::embedding_configuration()['endpoint'], $state['openai_endpoint'] );
