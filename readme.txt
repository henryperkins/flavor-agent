...
* WordPress developer-doc grounding uses Flavor Agent's built-in public Cloudflare AI Search endpoint during user-triggered recommendation requests. Site owners do not enter Cloudflare credentials for developer docs. Block, pattern, template, template-part, navigation, Global Styles, and Style Book recommendations require trusted current docs grounding before chat/reranking; content recommendations remain editorial-only and exempt.
...
* Cloudflare AI Search for WordPress developer docs — Used for trusted WordPress developer grounding through Flavor Agent's built-in public endpoint. Requests send docs-search queries derived from recommendation context, such as block names, template/navigation/style context, and the user's recommendation prompt, or administrator-supplied queries from the docs search ability. The accepted corpus includes stable Developer Handbook/reference pages plus current Developer Blog or Make/Core release-cycle posts with source and freshness metadata. Editor/functionality recommendations fail closed before chat when trusted grounding is unavailable; trusted stale/degraded guidance can proceed with warnings. Site owners do not enter Cloudflare credentials for developer docs. Cloudflare Terms: https://www.cloudflare.com/terms/ Privacy: https://www.cloudflare.com/privacypolicy/
