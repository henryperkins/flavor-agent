...
* WordPress developer-doc grounding uses Flavor Agent's built-in public Cloudflare AI Search endpoint during user-triggered recommendation requests. Site owners do not enter Cloudflare credentials for developer docs. Grounding is best-effort prompt context: when the docs backend is unreachable, recommendations still run without it and the editor shows a soft notice. Content recommendations remain editorial-only and do not use docs grounding.
...
* Connector-backed chat providers such as Anthropic or Google — Used through `Settings > Connectors` when the WordPress AI Client routes text generation to that connector. Flavor Agent sends the same recommendation prompt data described for text generation above. Anthropic Commercial Terms: https://www.anthropic.com/legal/commercial-terms Privacy: https://www.anthropic.com/privacy Google Gemini API Additional Terms: https://ai.google.dev/gemini-api/terms Google Privacy: https://policies.google.com/privacy
...
No external requests are made on activation. Activation installs the server-backed activity table and may schedule gated WP-Cron hooks, but optional roadmap guidance no-ops unless its explicit opt-in filter is enabled, and pattern sync runs only after the selected pattern backend is configured. There are no background docs-grounding crons.
