 ...
 * @return {any|null} Parsed JSON object.
 */
 ...
 * @return {boolean} True when the path exists and is a file.
 */
 ...
 * @return {Array<{name: string, commands: string[]}>} Matching script entries.
 */
function findPhpstanScripts( scripts ) {
	...
	for ( const [ name, raw ] of Object.entries( scripts ) ) {
		...
		const invokesPhpstan = commands.some( ( cmd ) => {
			...
			return (
				cmd.includes( 'phpstan' ) ||
				cmd.includes( 'vendor/bin/phpstan' )
			);
		} );
 ...
 * @return {{command: string|null, rationale: string}} Suggested command and why.
 */
 ...
 * @return {{mentionsScanDirectories: boolean, mentionsScanFiles: boolean}} Hints.
 */
 ...
 *
 * @param {string} configText Raw phpstan config contents.
 * @return {string[]} Unique, lowercased composer-style package references.
 */
 ...
 *
 * @return {Object} A stable, machine-readable inspection report.
 */
function buildReport() {
	const repoRoot = process.cwd();

	const composerPath = path.join( repoRoot, 'composer.json' );
	const composer = isFile( composerPath )
		? readJsonSafe( composerPath )
		: null;
	...
	if ( phpstanConfigFiles.length === 0 ) {
		notes.push(
			'No phpstan.neon or phpstan.neon.dist found at repo root.'
		);
	}
