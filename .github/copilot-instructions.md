...
Defines 35 abilities across block, pattern, template, navigation, docs, infra, content, style, and apply categories, including design inspection helpers. The plugin wires them via `wp_abilities_api_categories_init` and `wp_abilities_api_init`. Helper/read abilities and the six `preview-recommend-*` siblings register whenever the Abilities API and the WP AI plugin contracts are available (independent of the Flavor Agent feature gate). The eight recommendation abilities also require the Flavor Agent feature gate. Ten externally-useful read helpers and all six preview siblings declare `meta.mcp.public = true` so external MCP clients can discover them via the universal mcp-adapter default server; three helpers stay Abilities-API-only (`list-synced-patterns`, `get-synced-pattern`, `check-status`). On WP 7.0 admin screens, core auto-hydrates registered server-side abilities into the client-side `@wordpress/core-abilities` store. The canonical AI plugin's `Tools > Abilities Explorer` Experiment is the local harness for click-to-run testing — see `docs/reference/local-environment-setup.md`.
