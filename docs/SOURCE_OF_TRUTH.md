...
| Private Cloudflare AI Search     | Managed pattern indexing and retrieval                            | Pattern recommendations when the Cloudflare AI Search pattern backend is selected                                    | `flavor_agent_cloudflare_pattern_ai_search_instance_id`; account/token come from the Embedding Model settings                    |
...
- **Patterns** -- Pattern Storage selector, Qdrant URL/key, private Cloudflare AI Search pattern index name, backend-specific ranking thresholds, max results, and the `Sync Pattern Catalog` status/metrics/manual trigger panel. Pattern Storage is infrastructure, not another AI model choice.
...
Block, content, template, template-part, navigation, Global Styles, Style Book, and pattern reranking requests use the WordPress AI Client and `Settings > Connectors` for chat. Qdrant pattern storage uses the Cloudflare Workers AI Embedding Model. Saved provider values from older settings screens do not pin chat or select embeddings. The Cloudflare AI Search pattern backend uses the same Cloudflare account/token saved for the Embedding Model, then stores content in the configured private AI Search pattern index.
...
When the private Cloudflare AI Search pattern index name changes and the Embedding Model account/token are present, the settings save flow validates a filtered search probe and preserves the previous index name if validation fails.
