...
- **UI:** `src/template-parts/TemplatePartRecommender.js` renders a store-backed document settings panel with a compact scope bar, prompt composer, collapsed rationale, one `Recommendations` list, per-card detail disclosures, shared lower review panel, shared status notices, and collapsed recent activity. Non-deterministic ideas stay visible as compact `Manual` cards instead of disappearing.
...
- **UI:** Before a usable result, the full composer is visible. After a usable result, the panel becomes a Latest Recommendation Workspace: the generated output is primary, the composer collapses behind `Refine request`, supporting notes/issues collapse under `Editorial Notes`, recent diagnostics collapse under `Recent Content Requests`, and stale results keep a refresh banner while disabling copy.
...
- **Schema:** Block, template, template-part, Global Styles, and Style Book apply actions share one activity shape: surface, target identifiers, suggestion label, before/after state, prompt/reference metadata, timestamp, and undo status. The admin audit lane also stores scoped read-only `request_diagnostic` rows for successful and failed recommendation requests across content, pattern, navigation, block, template, template-part, Global Styles, and Style Book when a document scope exists; signature-only freshness probes do not create rows. Recommendation outcome diagnostics use `type: "recommendation_outcome"`, `diagnostic: true`, `executionResult: "diagnostic"`, `undo.status: "not_applicable"`, enum events/reasons, bounded strings, and hashed/signature request identifiers rather than raw prompts, generated text, block attributes, post content, or pattern payloads.
