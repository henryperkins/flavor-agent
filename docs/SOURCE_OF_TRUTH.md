...
flavor-agent/
  ...
  inc/                      PHP backend (PSR-4 namespace FlavorAgent\)
   ...
   AzureOpenAI/            Shared embedding clients, legacy Azure path, Responses facade, and Qdrant vector DB
...
| Provider selection               | Chooses OpenAI Native, Cloudflare Workers AI, or a connector path | Embedding backend selection, connector-backed chat pinning, and pattern ranking diagnostics                          | `flavor_agent_openai_provider` (`openai_native`, `cloudflare_workers_ai`, or a configured connector-backed provider)             |
...
- **LLM:** `ChatClient::chat()`; uses the WordPress AI Client runtime configured in `Settings > Connectors`, with saved legacy connector pins honored only when that connector is still available.
...
- **Pipeline:** Build query text -> cache-backed WordPress docs grounding via Cloudflare AI Search -> selected pattern retrieval backend -> LLM rerank via the active responses backend -> filter scores below the backend-specific ranking threshold -> return up to the saved max result count (default `8`, capped at `12`). Missing or empty `visiblePatternNames` returns an empty list before retrieval, docs, or ranker calls. The Qdrant backend uses provider-selected embeddings (OpenAI Native or explicitly selected Cloudflare Workers AI) plus two-pass Qdrant search (semantic + structural). The Cloudflare AI Search backend sends query text and the `visiblePatternNames` filter to the private pattern AI Search instance and does not call `EmbeddingClient` or `QdrantClient`.
...
- **Embedding Model** -- One Flavor Agent embedding provider selection (`openai_native` or `cloudflare_workers_ai`), OpenAI Native API key override/model/effective key source, and Cloudflare Workers AI account/token/model. Reasoning effort remains a backwards-compatible saved option used by the Connectors-routed chat runtime when present, but it is no longer an editable Embedding Model control. Text-generation provider and model readiness belong to `Settings > Connectors`.
...
- **Developer Docs** -- Built-in public Cloudflare AI Search developer-doc grounding, max result count, runtime grounding diagnostics, docs prewarm diagnostics, and legacy Cloudflare override controls only when saved override values already exist.
...
Block, content, template, template-part, navigation, Global Styles, Style Book, and pattern reranking requests use the WordPress AI Client and `Settings > Connectors` for chat. Direct OpenAI Native and Cloudflare Workers AI selections configure embeddings only; chat uses the configured WordPress AI Client runtime. Saved legacy connector selections pin chat to that connector only when it remains available, and otherwise fail closed. Qdrant pattern storage uses the single Flavor Agent Embedding Model. The Cloudflare AI Search pattern backend uses Cloudflare-managed embeddings/indexing inside a private AI Search instance instead. When OpenAI Native is selected for embeddings, the API key can be inherited from the core OpenAI connector unless a plugin-specific override is saved. Cloudflare Workers AI is an explicit embedding-provider selection and is not used as fallback when OpenAI Native or a legacy connector value is selected.
...
Successful saves still use the standard Settings API notice flow, and failed OpenAI Native, Cloudflare Workers AI, Qdrant, or Cloudflare AI Search validation surfaces a plugin-scoped error notice on the same screen.
...
- [x] Live credential validation on OpenAI Native, Cloudflare Workers AI, Qdrant, and Cloudflare AI Search settings save
