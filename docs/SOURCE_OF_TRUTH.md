...
flavor-agent/
  ...
  inc/                      PHP backend (PSR-4 namespace FlavorAgent\)
    ...
    AzureOpenAI/            Azure embedding/Responses clients + Qdrant vector DB
...
| Provider selection               | Chooses Azure OpenAI, OpenAI Native, Cloudflare Workers AI, or a connector path | Embedding backend selection, connector-backed chat pinning, and pattern ranking diagnostics                          | `flavor_agent_openai_provider` (`azure_openai`, `openai_native`, `cloudflare_workers_ai`, or a configured connector-backed provider) |
