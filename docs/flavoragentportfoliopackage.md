...
- Added a self-signed attestation layer for governed external style, template, and template-part applies, exposing strict-profile signed statements, durable public site keys, live subject-state digests, and bounded signed revert/supersession chains for independently checkable site-key proof.
