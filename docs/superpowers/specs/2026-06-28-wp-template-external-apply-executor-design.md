...
1. **Gate-1 — signature recompute (request):** recompute `resolvedContextSignature` + `reviewContextSignature` via `recommend_template` `resolveSignatureOnly`, forwarding `templateRef` / `templateType` / `prompt`, plus optional `visiblePatternNames`, `designSemantics`, `editorSlots`, and `editorStructure` **only when the caller supplied them** (the `array_key_exists` conditional, so absent keys never shift the signature). `hash_equals` against provided. This keeps the external lane aligned with the actual `recommend-template` signature contract: if a recommendation was grounded in visible-pattern filters, semantic hints, or live editor overlays, the same signed envelope must replay here or the request false-fails as stale. The external lane still stays fail-closed: it replays caller-supplied signed context for freshness, then re-validates execution against the live server-collected template contract before any write.
...
- **Signature-envelope coupling:** `recommend_template`'s signatures fold optional `visiblePatternNames`, `designSemantics`, `editorSlots`, and `editorStructure` into the resolved request envelope. Getting the replay wrong rejects valid recommendations as stale: omitting any optional key that was present in the original request breaks the hash, while unconditionally injecting absent keys also shifts the signature. **Test the signed replay both ways** — a `designSemantics`-bearing recommendation must apply, and an editor-overlay (`editorSlots`/`editorStructure`) recommendation must also apply when the same optional keys are replayed through the request ability.
