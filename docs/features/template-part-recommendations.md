...
- Two context bases exist and are internally consistent by design. In-editor recommendations are generated against the live editor overlay (including unsaved edits), and the in-editor apply mutates that same live tree, so they always agree. The external governed apply (`flavor-agent/request-template-part-apply` → `TemplatePartApplyExecutor`) instead re-collects the saved server document, and an external agent's recommendation also runs against the saved document, so those agree too. The one cross-basis case — an in-editor result produced against unsaved edits, then replayed through the external apply lane before the part is saved — is caught by the `resolvedContextSignature` gate and the executor's live re-validation, which fail closed with a re-validation error rather than applying against a document the suggestion never saw. Regenerate against the saved document to proceed; this is intended governance behavior, not a lost apply.
