...
|---|---|
| Block recommendations | `ChatClient::is_supported()` returns `true` when `WordPressAIClient::is_supported()` returns `true`. Chat goes through `Settings > Connectors`. |
| Pattern recommendations | Any configured direct embedding backend (`azure_openai` or `openai_native`), Qdrant configured, a usable WordPress AI Client chat runtime for ranking, and a usable pattern index |
