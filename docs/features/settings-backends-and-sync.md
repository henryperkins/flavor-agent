...
- Private Cloudflare AI Search pattern index name when Cloudflare AI Search Pattern Storage is selected. Account and token come from the Embedding Model fields.
...
| Pattern recommendations       | Selected pattern storage ready, WordPress AI Client chat for ranking, and a usable pattern index. Qdrant requires the single configured Embedding Model plus Qdrant. Cloudflare AI Search requires the Embedding Model account/token plus a private pattern index name.                                                                                                                     |
...
3. Flavor Agent validates Cloudflare Workers AI, Qdrant, and private pattern Cloudflare AI Search settings when those values changed and enough data is present to run the validation. Cloudflare AI Search pattern validation reuses the Embedding Model account/token with the saved pattern index name. Legacy Azure/OpenAI embedding options are not rendered or save-validated from the admin screen. Developer Docs uses the built-in public endpoint and has no credential save-validation path.
...
- Cloudflare AI Search pattern retrieval uses the private site-owner Cloudflare account/token saved for the Embedding Model plus the pattern index name, and is separate from the public WordPress developer-docs AI Search endpoint
