...
| Pattern recommendations       | Selected pattern storage ready, WordPress AI Client chat for ranking, and a usable pattern index. Qdrant requires the single configured Embedding Model plus Qdrant. Cloudflare AI Search requires valid Embedding Model credentials plus a managed `flavor-agent-patterns-{site_hash}` AI Search instance validated against the current account/token/model signature.                                                                                                                     |
...
7. Pattern setup messages describe storage readiness separately from model readiness so Pattern Storage does not look like another AI model picker. Cloudflare AI Search Pattern Storage states are: needs Embedding Model credentials, create managed pattern index, ready, and failed/incompatible.
