...
- Embedding Model: Cloudflare Workers AI account ID, API token, and effective embedding model for Flavor Agent semantic features. A blank model field uses the default Workers AI embedding model `@cf/qwen/qwen3-embedding-0.6b`.
- Azure OpenAI endpoint, API key, and embedding deployment are no longer editable settings. The neutral `flavor_agent_reasoning_effort` option is still registered and preserved for legacy/API-level use, but it is not currently exposed as a visible `Settings > Flavor Agent` control. Valid legacy `flavor_agent_azure_reasoning_effort` values are read only as a fallback/migration source, and runtime calls fall back to `medium` when neither saved value is valid. Text-generation provider and model readiness belong to `Settings > Connectors`.
- Cloudflare Workers AI account ID, API token, and optional embedding model ID.
...
- Qdrant URL and API key are editable in the Patterns section and are required only when Qdrant Pattern Storage is selected.
- Cloudflare AI Search Pattern Storage managed status when that backend is selected. Account, token, and a normalized AI Search embedding model come from the Embedding Model fields; blank or unsupported model values use `@cf/qwen/qwen3-embedding-0.6b` for the private pattern index, and legacy pattern-specific Cloudflare account ID, namespace, and token options are cleanup-only.
...
| WordPress docs grounding      | Built-in public Cloudflare AI Search endpoint; recommendation-time use checks exact-query, family, entity, and generic guidance caches first. Block, pattern, template, template-part, navigation, Global Styles, and Style Book requests may foreground-warm docs guidance on generic or missing fallback guidance before async warming is queued. If trusted grounding is unavailable, those functionality recommendation surfaces fail closed before chat/reranking; content recommendations remain editorial-only and exempt.                                    |
...
3. Flavor Agent validates Cloudflare Workers AI and Qdrant settings when those values changed and enough data is present to run the validation. When Cloudflare AI Search Pattern Storage is selected, Flavor Agent reuses the Embedding Model account/token and normalized AI Search embedding model to create or adopt the deterministic managed `flavor-agent-patterns-{site_hash}` instance; adopted instances must match the expected schema, owner marker, and normalized model, while newly created instances are created with the expected schema/model and then owner-marker validated. Legacy Azure/OpenAI embedding options are not rendered or save-validated from the admin screen. Legacy pattern-specific Cloudflare account ID, namespace, and token options are cleanup-only and do not override the Embedding Model values. Developer Docs uses the built-in public endpoint and has no credential save-validation path.
