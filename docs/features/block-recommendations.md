...
- Rollback ownership is established from requested client-ID presence before and after dispatch. Rollback never removes a requested-length positional slice, and a successful operation records its exact `insertedClientIds` or `replacementClientIds`.
...
11. When the user applies a reviewed structural suggestion, `applyBlockStructuralSuggestion()` runs the same around-the-await live gate and uses only the verified post-await `blockOperationContext` as mutation authority before Task 4's live-target, native-permission, recursive-clone, exact-ID rollback, and activity checks
...
- Structural insert/replace uses exact-ID rollback if the editor dispatch does not produce the expected parsed pattern blocks; zero insertion removes nothing, and partial insertion removes only newly present requested IDs
