...
- Structural apply is blocked when the reviewed target disappeared, changed block type or target signature, became locked or content-only, lost the referenced registered pattern, or no longer permits the requested insert/replace action. The structural target, action, pattern, editing mode, lock, and permission context comes from the exact post-revalidation live capture; stored or pre-await context is never fallback mutation authority.
