...
- Behind the flag, the prompt may propose at most one selected-block pattern operation. PHP validates every proposal before it reaches the normalized response; invalid, stale, content-only, cross-surface, unavailable-pattern, action-disallowed, and multi-operation proposals stay in `rejectedOperations` for diagnostics.
...
- Block apply freshness has three gates: the client-local request signature drives immediate stale UI; the stored server `resolvedContextSignature` hashes the server-normalized apply context plus the sanitized prompt; and an in-flight live editor-context stability check closes the interval around apply-time server revalidation. Single, batch, and structural apply bind revalidation to the thunk's exact target `clientId` and non-empty request `contextSignature`, capture the live context immediately before `resolveSignatureOnly: true`, and require the same non-empty live signature immediately after it resolves. A mismatch is client-stale and causes no editor mutation, activity, toast, or resolved-signature rebaseline. Background revalidation still silently demotes or disables results when the server signature drifts, while docs-cache warming alone does not invalidate apply.
- The panel states that inline apply is the exception for safe local block updates. Validator-approved structural block operations enter a review-first lane; applying them goes through a separate block-tree path with exact rollback for sibling insertion, atomic Core replacement, and drift-safe undo.
...
Applying suggestions from a stable fresh result does not stale that result: after the post-await live-context gate passes, `applySuggestion()` and the batch `applySelectedSuggestions()` thunk may adopt a pending server baseline and then re-baseline the stored client context to the post-apply block, so the user can keep applying without refreshing. A client-stale attempt cannot adopt or re-baseline itself.

## Structural Identity, Permissions, Rollback, And Session Undo

- Every parsed pattern block is passed through Core's `cloneBlock()` before identity capture. Core recursively creates fresh top-level and inner block client IDs, so repeated application of the same cached pattern cannot reuse the previous apply's identities.
- Sibling insertion preflights `canInsertBlockType()` for every parsed top-level block at the live parent/destination root. Replacement makes the same insertion check and additionally requires `canRemoveBlock()` to return `true` for the selected target.
- The selected block's `lock.move` and `lock.remove` attributes and a selected container's `templateLock` are not collapsed into a blanket target-lock decision. Core's live insertion and removal selectors are authoritative; Flavor Agent does not add custom ancestor-policy traversal. A movement lock therefore does not by itself prevent sibling insertion or replacement, while a removal denial prevents replacement.
- Sibling-insertion rollback ownership is established from requested client-ID presence before and after dispatch and never removes a requested-length positional slice. Replacement apply and inverse undo use Core's atomic `replaceBlocks()` dispatch with exact target/replacement IDs. A successful operation records its exact `insertedClientIds` or `replacementClientIds`.
- Structural undo requires those exact runtime IDs and the existing post-apply structural signature. It is intentionally limited to the editor runtime that created the IDs. After reload, the persisted structural activity remains audit history, but Gutenberg's regenerated IDs make the hydrated row non-undoable; there is no position, path, or snapshot fallback. Legacy or malformed structural activity without the exact IDs fails closed.
...
10. When the user applies an inline-safe single or batch attribute suggestion, the store verifies the stored client request signature, captures the request-bound live target/signature, re-posts the same request with `resolveSignatureOnly: true`, and verifies the live signature again immediately before synchronously merging allowed updates and recording activity
11. When the user applies a reviewed structural suggestion, `applyBlockStructuralSuggestion()` runs the same around-the-await live gate and uses only the verified post-await `blockOperationContext` as mutation authority before Task 4's live-target, native-permission, recursive-clone, exact-ID sibling-insertion rollback, atomic replacement, and activity checks
12. Inline undo validates the live block state before restoring the previous attribute snapshot. Structural undo validates the live structural signature before removing inserted blocks or atomically replacing exact replacement IDs with the recorded original snapshot.
...
- Apply is blocked when the request target or non-empty live signature does not match at preflight, when the live server-resolved apply context drifts, or when the editor context changes while server revalidation is in flight. Client-stale paths preserve the user's edit and produce no editor mutation, activity, toast, adoption, or rebaseline.
- If no allowed attribute updates remain after validation, the suggestion is not applied
- Structural apply is blocked when the reviewed target disappeared, changed block type or target signature, became content-only, lost the referenced registered pattern, or Core's live action-specific selectors no longer return `true` for the requested insert/replace action. The structural target, action, pattern, editing mode, and permission context comes from the exact post-revalidation live capture; stored or pre-await context is never fallback mutation authority.
...
- Structural sibling insertion uses exact-ID rollback if the editor dispatch does not produce the expected parsed pattern blocks; zero insertion removes nothing, and partial insertion removes only newly present requested IDs. Replacement uses Core's atomic `replaceBlocks()` dispatch for apply and inverse undo; a declined/no-op replacement leaves the pre-operation tree unchanged.
...
- Structural undo is blocked if the exact recorded runtime IDs are unavailable or the live root structure no longer matches the recorded post-apply structural signature. Reloaded and legacy/malformed activity stays visible for audit but fails closed as non-undoable.
