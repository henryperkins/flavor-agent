...
9. For every eligible external style, template, or template-part apply, the activity row durably records whether attestation was `recorded`, `not_configured`, or `failed`; a successful undo records its own `recorded`, `not_configured`, `failed`, or `not_applicable` result. The detail view uses bounded operator copy for those outcomes and never exposes raw exception text
10. For a recorded attestation, the detail view exposes the attestation id, a strict site-run verification summary, raw signed-statement and subject-state URLs, and any revert/supersede chain context so a reviewer can move from wp-admin audit evidence to external verification without confusing endpoint loading for cryptographic verification. Invalid results render as errors; honest drift and incomplete verification render as warnings
...
- Let admins discover Ring III governed-change attestations for eligible external style, template, and template-part applies, including strict status/severity, terminal chain context, a site-run verification summary, and public envelope/subject-state links without making attestation a general AI-governance claim
- Explain eligible apply/undo recording outcomes honestly when no key is configured, recording fails, or an undo has no original attestation to chain from
