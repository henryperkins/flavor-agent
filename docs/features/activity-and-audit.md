...
- Inline editor activity: `Recent AI Actions` inside the block, template, and template-part recommendation panels; `Recent AI Style Actions` inside Global Styles; `Recent AI Style Book Actions` inside Style Book; and read-only `Recent Content Requests` inside the content panel. Block request diagnostics can also appear inline in the block activity section when a request fails or returns no block-lane suggestions.
...
Navigation, pattern, and the content recommendation panel do not create executable apply-and-undo entries because they do not run Flavor Agent-owned apply flows. The server does, however, persist scoped read-only `request_diagnostic` rows for successful and failed recommendation fetches across content, pattern, navigation, block, template, template-part, Global Styles, and Style Book when a document scope is available. On executable surfaces, those request rows are separate from any later apply-and-undo activity row. Content request diagnostics appear inline as `Recent Content Requests`; block request diagnostics can appear inline when a block request fails or returns no block-lane suggestions; and scoped template, template-part, Global Styles, and Style Book diagnostics can hydrate into matching recent-activity lists. Navigation and pattern diagnostics appear in the wp-admin audit page.

## Surfacing Conditions

- Inline activity appears only when the current editor scope has recorded executable block, template, template-part, Global Styles, or Style Book AI actions, or read-only diagnostics for content, block, template, template-part, Global Styles, or Style Book requests
...
1. A deterministic apply flow succeeds in the block, template, template-part, Global Styles, or Style Book surface, or a scoped content, block, template, template-part, Global Styles, or Style Book request succeeds, fails, or returns diagnostic-only output
...
- Persist read-only `request_diagnostic` audit rows for scoped recommendation fetches across all recommendation surfaces without pretending advisory-only surfaces now support executable apply/undo; content shows those rows inline as `Recent Content Requests`, block can prepend diagnostics to `Recent AI Actions` for failed or empty block-lane requests, executable surfaces can hydrate matching scoped diagnostics into their recent-activity lists, and navigation/pattern diagnostics remain audit-feed only
...
- Show ordered undo state: persisted undo status values are `available`, `review`, `undone`, and `failed`; runtime/admin projections can derive `blocked` when newer still-applied actions prevent undo. Persistence sync states (`server`/`local` with `syncType` of `undo` or `create`) are tracked separately
...
This is still the first audit surface, not the final observability product. It includes structured diff and before/after summaries, but not a rich visual diff viewer, richer operator workflows, row actions/discovery, or a broader diagnostics console.
...
| Admin UI | `src/admin/activity-log.js` | Renders the DataViews feed, summary cards, and custom read-only detail sections |
