...
- Filter audit entries by absolute or relative time without silently broadening malformed date filters; `inThePast` and `over` use true timestamp windows, including hour-based filters that cross midnight correctly
