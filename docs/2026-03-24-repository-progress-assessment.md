...
> Historical snapshot: this assessment is retained for execution lineage from the 2026-03-24 docs pass. Use `STATUS.md`, `docs/SOURCE_OF_TRUTH.md`, and `docs/2026-03-25-roadmap-aligned-execution-plan.md` for the current verified backlog.
>
> Current-truth note: the status labels and gap descriptions below are anchored to 2026-03-24 and can describe work that later landed. If this file conflicts with the newer docs, the newer docs win.

## Executive Summary

Flavor Agent is a late-prototype / early-beta plugin with real shipping code, not a scaffold. It is close to a hardened v1 for its five current editor surfaces: block inspector recommendations, pattern recommendations, advisory navigation recommendations, template recommendations, and a narrow executable template-part recommendation flow are all implemented in current code, wired into the editor, and backed by passing PHP and JS unit suites. The main remaining gaps are narrower than this document originally captured: navigation now has a first-party inspector surface and plugin REST adapter, but it remains advisory-only and lacks browser smoke plus a validated apply contract; activity/auditability is now server-backed, but still lacks a mature admin/audit UX; template-part execution is intentionally limited to deterministic start/end pattern insertion; and WordPress 7.0 support still depends on explicit compatibility adapters plus split browser verification where the default `npm run test:e2e` path remains on the WordPress `6.9.4` Playground harness while a separate Docker-backed `npm run test:e2e:wp70` command owns the active refresh/drift coverage. There is still no checked-in browser smoke for the `wp_template_part` apply path. The absence of runtime Interactivity API usage is an explicit non-goal for the current editor/admin-only plugin, not a shipping gap.
...
- **[partial] Navigation recommendations.** Selected `core/navigation` blocks now surface a first-party Inspector panel backed by `POST /flavor-agent/v1/recommend-navigation` and the shared store. The surface is intentionally advisory-only today and still lacks browser smoke plus a bounded apply contract. Evidence: `src/inspector/NavigationRecommendations.js`, `src/inspector/InspectorInjector.js`, `src/store/index.js`, `inc/REST/Agent_Controller.php`, `flavor-agent.php`. Coverage: `tests/phpunit/AgentControllerTest.php`, `src/inspector/__tests__/NavigationRecommendations.test.js`.

- **[partial] AI activity history and undo.** The activity system is implemented for block, template, and template-part apply flows, persists through the server-backed activity repository, hydrates per edited entity into editor-scoped history, and keeps `sessionStorage` only as a fast cache/fallback. Ordered undo now follows the newest valid AI-action tail, with older entries blocked when newer AI actions remain. The remaining gap is product maturity: there is still no dedicated admin audit screen or broader observability UX, and default browser verification still leaves both template-part browser coverage and the WP 7.0 Site Editor refresh/drift undo cases outside `npm run test:e2e` even though the latter now exist in the separate `npm run test:e2e:wp70` harness. Evidence: `inc/Activity/Repository.php`, `inc/REST/Agent_Controller.php`, `src/store/activity-history.js`, `src/components/ActivitySessionBootstrap.js`, `src/components/AIActivitySection.js`, `src/store/index.js`, `src/utils/template-actions.js`, `src/templates/TemplateRecommender.js`, `src/template-parts/TemplatePartRecommender.js`, `package.json`, `playwright.config.js`, `playwright.wp70.config.js`. Coverage: `src/store/__tests__/activity-history.test.js`, `src/store/__tests__/store-actions.test.js`, `src/utils/__tests__/template-actions.test.js`, `tests/e2e/flavor-agent.smoke.spec.js`.
...
| `flavor-agent/recommend-navigation` | `edit_theme_options` | yes | no | `NavigationAbilities::recommend_navigation` | partial (advisory-only) |
...
- **[partial] `recommend-navigation` is now exposed through both the plugin REST controller and a first-party Inspector panel.** The remaining gap is narrower: the shipped surface is still advisory-only, and I did not find checked-in browser smoke or a validated apply contract for it. Evidence: `inc/Abilities/NavigationAbilities.php:23-62`, `inc/Context/ServerCollector.php:1168-1249`, `inc/REST/Agent_Controller.php:98-124,399-429`, `src/inspector/InspectorInjector.js:32-35,355-355`, `src/inspector/NavigationRecommendations.js`.

- **[implemented] Navigation is no longer Abilities-only.** The plugin REST controller now exposes block, pattern, navigation, template, template-part, activity, and sync routes, while docs search remains an Abilities-only admin surface. Evidence: `inc/REST/Agent_Controller.php:17-275`.
...
- **[implemented] Editor bootstrapping is centralized in `src/index.js`.** The plugin registers a store-backed root that loads activity hydration, pattern recommender UI, the shared Inspector injection (including navigation on `core/navigation`), inserter badge, template panel, and template-part panel. Evidence: `src/index.js:9-38`, `src/inspector/InspectorInjector.js:7-29,59-462`.
...
- **[implemented] The REST controller is a thin adapter over abilities, not a separate business-logic layer.** It sanitizes request data and forwards into ability handlers for block, pattern, navigation, template, template-part, activity, and sync flows. Evidence: `inc/REST/Agent_Controller.php:17-275,332-576`. Coverage: `tests/phpunit/AgentControllerTest.php`.
...
- **[docs-only] This file is now historical execution context, not the live backlog list.** Use `STATUS.md`, `docs/SOURCE_OF_TRUTH.md`, and `docs/2026-03-25-roadmap-aligned-execution-plan.md` for the current verified gap framing.
...
- **[implemented] Top-level docs are now broadly aligned with the current codebase.** `STATUS.md`, `docs/SOURCE_OF_TRUTH.md`, and `docs/2026-03-24-review-follow-up-plan.md` now match the implemented block, pattern, navigation, template, settings, docs-grounding, and WP 7.0 compatibility posture. The major remaining backlog is narrower: navigation apply/browser-smoke follow-through, activity audit/admin UX maturity, expanded template-part execution, WP 7.0 adapter hardening, and unified browser verification with template-part smoke coverage.
...
1. **Add browser smoke for the shipped navigation surface and decide whether it should stay advisory-only or grow a bounded apply contract.** The inspector panel and REST adapter are now real; the remaining work is contract clarity plus verification.
2. **Promote the server-backed activity layer into an admin-visible audit and observability surface.** The current undo and recent-actions model now persists through the backend, but cross-device review and dedicated admin UX are still absent.
