...
Flavor Agent is a late-prototype / early-beta plugin with real shipping code, not a scaffold. It is close to a hardened v1 for its four strongest editor surfaces: block inspector recommendations, pattern recommendations, template recommendations, and now a narrow executable template-part recommendation flow are all implemented in current code, wired into the editor, and backed by passing PHP and JS unit suites. The main gaps are that history/auditability is still session-scoped, template-part execution is intentionally limited to deterministic start/end pattern insertion, the WordPress Interactivity API is not used in runtime code, and WordPress 7.0 support still carries a few Gutenberg-compatibility risks because some runtime paths depend on experimental fallbacks or browser-harness limitations rather than stable end-to-end verification. The documentation drift identified earlier in this review has now been corrected in the top-level docs.

Verification for this assessment used the requested repo docs plus direct code inspection in `flavor-agent.php`, `inc/`, `src/`, and `tests/`. In this pass, `vendor/bin/phpunit` passed (`149` tests, `767` assertions) and `npm run test:unit -- --runInBand` passed (`21` suites, `157` tests). I inspected the checked-in Playwright smoke coverage but did not re-run it in this pass.
...
- **[partial] AI activity history and undo.** The activity system is implemented for block, template, and template-part apply flows, is scoped per edited entity in `sessionStorage`, and supports inline undo plus recent-actions UI. Only the most recent AI action is auto-undoable, there is no server-backed audit log, and the checked-in browser smoke coverage still leaves both template-part browser coverage and the Site Editor refresh/drift undo cases outside the verified Playwright path. Evidence: `src/store/activity-history.js`, `src/components/ActivitySessionBootstrap.js`, `src/components/AIActivitySection.js`, `src/store/index.js`, `src/utils/template-actions.js`, `src/templates/TemplateRecommender.js`, `src/template-parts/TemplatePartRecommender.js`. Coverage: `src/store/__tests__/activity-history.test.js`, `src/store/__tests__/store-actions.test.js`, `src/utils/__tests__/template-actions.test.js`, `tests/e2e/flavor-agent.smoke.spec.js`.
...
- **[partial] The local browser-verification story is weaker than the declared support floor.** The repo’s checked-in smoke file contains working post-editor and Site Editor coverage, but the historical status notes keep the stable Playground harness on WordPress `6.9.4`, and two Site Editor refresh/drift tests remain `fixme`. That means WP 7.0 support is credible in code shape, but not yet fully exercised end-to-end in the checked-in browser harness. Evidence: `STATUS.md:54-60`, `tests/e2e/flavor-agent.smoke.spec.js:855-975`.
...
- **[partial] Browser coverage still stops short of full WP 7.0 Site Editor confidence.** The checked-in smoke suite verifies block, pattern, and template flows, but the refresh/drift Site Editor undo cases remain `fixme` and the project notes still describe the stable Playground harness as WordPress `6.9.4`. Evidence: `tests/e2e/flavor-agent.smoke.spec.js:515-975`, `STATUS.md:54-60`.
...
- **[implemented] Top-level docs are now broadly aligned with the current codebase.** `STATUS.md`, `docs/SOURCE_OF_TRUTH.md`, and `docs/2026-03-24-review-follow-up-plan.md` now match the implemented block, pattern, template, settings, docs-grounding, and WP 7.0 compatibility posture. The major remaining gaps are browser-harness confidence for WP 7.0 Site Editor flows, template-part browser smoke coverage, and the product decision around durable activity history.

## Top 5 Recommended Next Actions

Status note as of 2026-03-24 in the current source tree: actions 1 and 2 are now implemented; the open follow-up backlog starts at actions 3-5.

1. **[implemented] Add direct backend PHPUnit coverage for pattern recommendation and indexing paths.** `PatternAbilities` and `PatternIndex` are now covered by dedicated PHPUnit suites.
2. **[implemented] Add a panel/store-level test for template context drift.** Helper tests still cover normalization and signature stability, and `src/templates/__tests__/TemplateRecommender.test.js` now proves the live template panel clears stale recommendations when insertion-root scope changes.
3. **Restore true WordPress 7.0 browser verification for Site Editor flows.** Unskip the refresh/drift template undo Playwright cases once a stable 7.0 harness is available, because this is the largest remaining confidence gap between declared support and end-to-end verification.
4. **Add browser smoke coverage for the new template-part apply/undo flow.** The narrow executable surface is unit-covered, but it is not yet represented in the checked-in Playwright smoke path.
5. **Decide whether AI activity history should remain session-scoped or move to a server-backed audit trail.** The current undo and recent-actions model works for same-session editor flows, but cross-session review and durable auditability are still absent.
