...
- **Settings:** `composer run test:php -- --filter '(SettingsTest|ProviderTest|EmbeddingBackendValidationTest|AgentControllerTest|InfraAbilitiesTest)'`; `npm run test:unit -- --runInBand src/admin/__tests__/settings-page-controller.test.js src/utils/__tests__/capability-flags.test.js`; `npx playwright test tests/e2e/flavor-agent.settings.spec.js`.
