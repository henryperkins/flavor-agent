...
| [Admin AI Activity](features/activity-and-audit.md) | `Settings > AI Activity` | Current user has `manage_options` | Hidden for non-admin users; read-only today | Browse the recent server-backed activity feed, search and filter it, adjust the DataViews view configuration, inspect DataForm details, jump back to affected entities and settings | Audit only; no apply or row-action undo |
