...
- External agent style apply -> `flavor-agent/request-style-apply`; external agent template-part apply -> `flavor-agent/request-template-part-apply`; status/attribution reads -> `flavor-agent/get-activity` and `flavor-agent/list-activity`; server-side undo (both lanes) -> `flavor-agent/undo-activity`
