...
- Connectors-first chat ownership unchanged. `LLM\WordPressAIClient` routes recommendation chat through the WordPress AI service / `wp_ai_client_prompt()` fallback and the WordPress AI Client; the plugin still does not manage chat credentials.
- Plugin-owned embedding/Qdrant routing resolves through `OpenAI\Provider`, `Embeddings\EmbeddingClient`, `Embeddings\QdrantClient`, and `Cloudflare\AISearchClient` under `Settings > Flavor Agent`.
...
- Current Cloudflare docs grounding for the pattern surface may perform one bounded foreground warm and fails closed before reranking when trusted grounding remains unavailable.
