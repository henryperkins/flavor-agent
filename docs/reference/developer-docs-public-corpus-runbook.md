...
Current release decision: for the `v0.1.0` target release environment, corpus ownership and the refresh cadence below are accepted. The current WordPress 7.1 validation passed on 2026-08-26; its query, source URLs, freshness metadata, updater identity, and local artifact checksums are recorded in [`../validation/2026-08-26-public-corpus-validation.md`](../validation/2026-08-26-public-corpus-validation.md). There is no runtime coverage gate or grounding constant to configure: grounding is best-effort (`AISearchClient::maybe_search_best_effort`), never blocks a recommendation, and at runtime applies only structural URL hygiene plus non-gating source labels. Trust and currency of the corpus are owned by `scripts/update-docs-ai-search.js` at ingestion time.
...
The preferred updater is the scheduled/manual GitHub Actions workflow at `.github/workflows/update-docs-ai-search.yml`. It restores the prior `output/docs-ai-search/manifest.json` from the Actions cache, runs `npm run docs:ai-search:update -- --release=7-1`, discovers trusted source URLs, filters release-cycle listing/archive URLs out of the desired corpus, uploads changed Markdown items into the public Cloudflare AI Search corpus on `wp-dev-docs` using bounded source keys shaped like `ai-search/wp-dev-docs/{host}/{path-slug}/{short-hash}/part-0001.md` (capped at 128 bytes to satisfy Cloudflare's item-filename limit; the full canonical URL and content hash travel in item metadata, not the key), polls the instance `/stats` endpoint until queued/running/outdated counts settle and re-sweeps the item listing to verify desired keys (the `/stats` counters can read zero while the listing still reports a key as queued, so the sweep is retried inside the window rather than being a single shot), validates the public endpoint, and — only when stale deletion is enabled, the destructive full-run safety checks pass (no discovery/upload/poll problems, build errors remain within the 2% tolerance below, it is not a targeted run, and the prepared count has not regressed against the restored manifest or the live completed-corpus baseline) — removes stale managed docs items. Public endpoint validation makes up to `--validation-attempts` total probes (default 5, exponential backoff of 10/20/40/80 seconds between them) because Cloudflare resyncs its retrieval index asynchronously after ingest and answers a mid-resync query with HTTP 200 and zero or partial chunks. A probe is usable only when it returns both a current stable `developer-docs` chunk and a current `make-core` or `developer-blog` chunk under the freshness rules below; expected source labels without qualifying timestamps do not pass. If all five probes fail, one further diagnostic probe runs without `return_on_failure` so the summary records Cloudflare's real retrieval error rather than a silent HTTP 200; that probe is reported under `validation.diagnostic` and is not counted as a validation attempt. A validation failure that still returned chunks marks the run as needing attention without blocking deletion, because stale generations can be the reason validation is noisy; a failure that returned **zero** chunks blocks *both* destructive paths — bulk stale deletion and superseded same-source generation deletion — with reason `validation-unavailable`, since the run has no evidence the corpus is retrievable at all. The workflow is scheduled daily: only the Monday run passes `--delete-stale`, the other daily runs ingest without pruning, and manual dispatch remains opt-in through the `delete_stale` input. Instance configuration is skipped by default because Cloudflare treats config updates as an instance-wide resync; use the workflow's `configure_instance` input or the script's `--configure` flag only for deliberate metadata/search-config changes. Configure these repository secrets before enabling scheduled writes:
...
Dated-post recency is tunable with `--recent-post-max-age-days=<n>` (default 180; `0` ingests every matched dated post; `--make-core-max-age-days` is a deprecated alias). Script-level stale deletion remains explicitly gated by `--delete-stale`: the Monday schedule passes it automatically, while local and manual workflow runs only prune when the flag/input is set. Even when enabled it is skipped for `--limit`, targeted `--source-url`/`--source-file` runs, any sitemap/discovery error, skipped polling, item/upload failures, a build-error ratio above 2% of discovered URLs (the persistent sub-1% noise from binary attachment pages in the sitemaps does not block pruning), or a prepared-count regression versus the larger of the cached manifest length and the live count of distinct completed managed `source_url` values. A public endpoint validation failure that still returned chunks is recorded as a warning and keeps the summary in a needs-attention state, but does not block pruning after the destructive-safety checks have passed. A validation response with zero chunks blocks both destructive paths outright (`validation-unavailable`): the bulk stale deletion resolved by `resolveStaleDeletion()`, and the earlier deletion of superseded same-source generations resolved by `resolveSameSourceDeletion()`. The two resolvers deliberately share the same retrieval-evidence rule — item-level settlement proves a replacement was ingested, not that it is retrievable, so swapping generations while the index is dark risks leaving a source with no searchable copy. Validation only fails after every `--validation-attempts` probe is exhausted, and the summary's `validation.attempts` field records how many probes the run needed — a rising count is the early warning that index resync is getting slower. Targeted runs replace sitemap discovery and never delete, so they are safe for spot-checks. Because item keys include a short content hash, extraction or source-content changes create a new item generation; after a full healthy run uploads the desired generation, run a full `delete_stale` workflow or let the next healthy scheduled run prune older managed generations from the public corpus. The same full `delete_stale` pass also prunes previously managed release-cycle listing/archive pages such as `developer.wordpress.org/news/`, `developer.wordpress.org/news/all-posts/`, and `make.wordpress.org/core/`, because they are no longer part of the desired corpus.

**Settlement waiting is adaptive, not tolerant.** The base window is `--poll-seconds` (default 180; the workflow passes 600). Past it, the poll continues only while the run is demonstrably converging: every new low in the active item count restarts a `--poll-progress-grace-seconds` window (default 120), and a plateau expires on its own rather than consuming the remaining budget. `--poll-max-seconds` (default 1800) is an absolute stop; it can never shorten a larger explicitly requested `--poll-seconds`. Success still requires zero missing, zero pending, and zero errored desired keys, resolved by a final item-level sweep after the window closes — extension buys time for a slow ingest, it never reclassifies a stuck one as settled. Item statuses of `error`/`skipped` are terminal and stop the poll immediately, since waiting cannot clear them. Any residual pending item keeps `poll.pending > 0`, which blocks both destructive deletion paths and fails the workflow.

Each run records its settlement trajectory under `settlement` in the summary artifact: `polls`, `elapsedMs`, `initialActive`, `bestActive`, `finalActive`, `lastProgressAgeMs`, and `extended`. Read these before treating a pending item as a Cloudflare fault — `bestActive` equal to `finalActive` with a large `lastProgressAgeMs` is a stuck item, whereas a still-falling count that hit `elapsedMs` at the hard cap means the cap is too low for the batch size. Run 31462270794 (2026-08-11) failed with one pending item out of 126 uploaded and healthy validation; the summary of the day recorded only the endpoint, which is why the trajectory fields exist.
...
1. Set the active release identifier for the cycle, such as `7-1`.
...
3. Add or refresh the release-cycle source set for the active identifier:
   - the Make/Core release hub, for example `https://make.wordpress.org/core/7-1/`
   - the Make/Core dev-note tag, for example `https://make.wordpress.org/core/tag/dev-notes-7-1/`
...
4. Keep supporting sources such as Make/Test, Make/Playground, and Gutenberg GitHub releases in separate research snapshots. They can help humans decide what to ingest, but they are not part of the ingestion allowlist (`TRUSTED_ROOTS` in `scripts/update-docs-ai-search.js`). Make/AI and WordPress News joined the allowlist with the 2026-07-12 daily-ingest change.
5. Remove or deprioritize superseded release-cycle source entries when their `published_at` date can no longer pass the freshness windows or the active WordPress release-date rule below. Retaining them as historical search context is acceptable only if current release-cycle sources still rank for the validation query.
...
For corpus-currency validation, `make-core` chunks count as current when they are published within 21 days of validation, and `developer-blog` chunks within 45 days. `make-ai` and `wordpress-news` chunks count as current when published within 21 days of validation. A release-cycle chunk also counts when its `published_at` is on or after the active release's public-release floor: May 20, 2026 for WordPress 7.0 and August 19, 2026 for WordPress 7.1. Recrawling an older release post does not make it current. Stable `developer-docs` chunks may use crawl freshness and count as current for 90 days.
...
Corpus validation:
- Run the validation query: `WordPress 7.1 block.json metadata reference Gutenberg 23.8`.
- The query deliberately combines a stable Developer Docs identifier (`block.json metadata reference`) with the active release-cycle retrieval anchor (`WordPress 7.1` / `Gutenberg 23.8`). Update both release-cycle terms when the active cycle changes; do not remove the stable identifier or the mixed-source validation loses its handbook/reference half.
