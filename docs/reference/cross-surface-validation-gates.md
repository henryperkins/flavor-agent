...
| 2. Provider and backend routing | Provider selection, settings validation, `ResponsesClient`, `EmbeddingClient`, Qdrant, or Cloudflare grounding changed | Validate the selected provider path, direct-provider fallback, connector-backed path when applicable, and the block-only WordPress AI Client fallback; pattern work must validate chat, embeddings, and Qdrant together |
...
| 5. Shared UI taxonomy and mode | Shared UI components or prompt taxonomy changed | Validate that each touched surface still keeps its intended mode: block `Apply now`, template/template-part/style `Review first`, navigation advisory-only, patterns ranking-only, block Styles projection-only |
