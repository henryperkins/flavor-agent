...
Server-side review-freshness helper shared by template, template-part, style, and navigation recommendation abilities. `RecommendationReviewSignature::from_payload()` hashes a stable normalized payload of docs-free server review context only, so review freshness tracks real server-owned drift instead of docs guidance or grounded prompt text churn.
