...
Node runtimes for this repo must be **major 20 or 24** (matching `package.json` `engines.node: ^20 || ^24`), with **24** as the default selector via both `.nvmrc` and `.node-version`.
