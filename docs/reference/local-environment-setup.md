...
- **Before enabling the Flavor Agent AI feature, the Explorer should list the 18 always-on helper and preflight abilities.** That set is the twelve helper/read abilities, the docs search, and the five `preview-recommend-*` siblings. After enabling the Flavor Agent feature in `Settings > AI`, the seven `recommend-*` abilities register too, bringing the full list to 25 abilities.
