...
| `DELETE /flavor-agent/v1/activity/{id}/claim` | `manage_options` AND the row's mutation capability (`can_decide_activity_request`) | `src/admin/activity-log.js` approvals UI | `FlavorAgent\Apply\ApplyClaim::release()` | Releases the advisory review claim acquired via `POST .../claim`; advisory only, never gates a decision |
