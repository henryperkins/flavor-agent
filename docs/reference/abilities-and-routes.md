...
| `flavor-agent/undo-activity`           | Contextual per row (style, template, and template-part rows: `edit_theme_options`)                              | Feature-gated; dedicated MCP server only                                                                                                                                                                                                                                                                                                                                                                      | Server-side ordered undo with drift checks for executed style, template, and template-part rows (reversed through the executor registry)                                                                                                                                                                                                                                          | No first-party editor UI; external-agent surface                     |
...
- Public attestation routes are unauthenticated by design. They expose only public-safe governed-change statement data, a key registry, the site-served verification summary used by wp-admin, and the minimal canonical style subject needed to verify whether an attested external style apply still matches the live site. They do not expose prompts, provider payloads, user display names, raw full config blobs, or general activity history.
