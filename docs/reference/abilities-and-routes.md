...
- The Abilities API is the sole active contract for the eight `recommend-*` surfaces when the WordPress AI plugin contracts are available and the Flavor Agent AI feature is enabled
- Activity creation, admin decisions, and manual pattern sync remain REST routes. Activity read/list/undo for governed external applies are exposed as abilities via `get-activity`, `list-activity`, and `undo-activity`
- Ring III governed-change attestations are public REST reads, not abilities: `attestations/{id}`, `attestations/{id}/verification`, `attestations/{id}/subject-state`, and `attestations/keys` let reviewers inspect a site-served verification summary and let external verifiers inspect signed statements for Flavor Agent's style, template, and template-part lanes without wp-admin access. Verification binds the exact v1 statement profile to the requested site/id and follows only signed, digest-continuous revert/supersession chains (maximum 50 total records). Post-blocks remains excluded because its potentially non-public content has no safe public subject-state contract
...
| `flavor-agent/recommend-template`      | `edit_theme_options`                                                                                              | Connectors text-generation provider configured; developer-docs grounding is attached best-effort                                                                                                                                                                                                                                                                                                                  | Template suggestions plus validated template-part operations and bounded pattern insertions with explicit placement, optional anchor metadata, and `docsGrounding`                                                                                                                                                        | Site Editor template panel                                           |
| `flavor-agent/recommend-template-part` | `edit_theme_options`                                                                                              | Connectors text-generation provider configured; developer-docs grounding is attached best-effort                                                                                                                                                                                                                                                                                                                  | Template-part suggestions, focus blocks, patterns, validated bounded operations constrained by executable paths and anchors, and `docsGrounding`                                                                                                                                                                          | Site Editor template-part panel                                      |
| `flavor-agent/recommend-post-blocks`   | `edit_posts`; any positive post ID resolved from request/document context also requires `edit_post` for that post | Connectors text-generation provider configured; developer-docs grounding is attached best-effort                                                                                                                                                                                                                                                                                                                  | Structural suggestions for one post/page document: focus blocks, patterns, validated bounded operations constrained by a server-collected, lock-aware executable-target contract, echoed `operationTargets`/`insertionAnchors`, and `docsGrounding`                                                                       | No first-party editor UI; external-agent surface                     |
| `flavor-agent/recommend-style`         | `edit_theme_options`                                                                                              | Connectors text-generation provider configured; developer-docs grounding is attached best-effort                                                                                                                                                                                                                                                                                                                  | Shared style suggestions for Global Styles and Style Book, constrained to validated `theme.json` paths, theme-backed values, Global Styles-only theme variations, and `docsGrounding`                                                                                                                                    | Site Editor Global Styles and Style Book panels                      |
| `flavor-agent/list-template-parts`     | `edit_posts` or `edit_theme_options`                                                                              | None beyond capability                                                                                                                                                                                                                                                                                                                                                                                        | Registered template parts with canonical `id` (the `templatePartRef` accepted by `recommend-template-part`), plus slug/title/area and optional content for theme-capable callers                                                                                                                                       | No direct first-party UI; helper and external-agent surface          |
...
| `flavor-agent/recommend-navigation`    | `edit_theme_options`                                                                                              | Connectors text-generation provider configured for useful output; developer-docs grounding is attached best-effort                                                                                                                                                                                                                                                                                                | Advisory navigation suggestion groups, explanation, and `docsGrounding`                                                                                                                                                                                                                                                   | Navigation guidance inside the block panel                           |
| `flavor-agent/search-wordpress-docs`   | `manage_options`                                                                                                  | Uses Flavor Agent's built-in public WordPress Developer Docs AI Search endpoint; execution fails closed for missing queries, invalid endpoint config, or HTTP/search/parse errors; chunks are kept with non-gating source labels (trust is owned by the ingestion script) and empty results return an empty `guidance` array with `docsGrounding.available: false`                                                             | WordPress developer-doc guidance, `docsGrounding`, and `docsGroundingFingerprint`                                                                                                                                                                                                  | No direct first-party editor UI; admin and external-agent surface    |
...
| `flavor-agent/get-theme-styles`        | `edit_posts` or `edit_theme_options`                                                                                                    | None beyond capability                                                                                                                                                                                                                                                                                                                                                                                        | Applied global theme styles plus extracted element and pseudo-state summaries, along with the live Global Styles `scope` and `styleContext` payload external callers can forward into `recommend-style` / `request-style-apply`                                                                                       | No direct first-party UI; helper and external-agent surface          |
...
| `flavor-agent/check-status`            | `edit_posts`                                                                                                      | None beyond capability                                                                                                                                                                                                                                                                                                                                                                                        | Backend inventory, active model hint, currently available ability list, per-surface readiness map, and `agentRuntime` (optional Agents API readiness, reported separately from `backends`)                                                                                                                                                                                                                     | Settings diagnostics and external-agent surface                      |
...
| `flavor-agent/preview-recommend-post-blocks` | `edit_posts`; any positive post ID resolved from request/document context also requires `edit_post` for that post | None beyond capability (delegates to `flavor-agent/recommend-post-blocks`'s permission_callback)                                                                                                                                                                                                                                                                                                              | Signature-only preflight: returns `reviewContextSignature` and `resolvedContextSignature`. Forces `resolveSignatureOnly:true` and strips `clientRequest` server-side                                                                                                                                                      | No first-party editor UI; Abilities Explorer and external MCP surface |
...
| `flavor-agent/request-template-part-apply` | `edit_theme_options`                                                                                          | Feature-gated; exposed on the dedicated MCP server only                                                                                                                                                                                                                                                                                                                                                       | Queues a review-gated template-part structural apply (≤3 path-addressed bounded operations against one `wp_template_part`) from a `recommend-template-part` result and re-checks freshness at request; mutates nothing until a site admin approves it in `Settings > AI Activity`                                          | No first-party editor UI; external-agent surface                     |
| `flavor-agent/request-post-blocks-apply` | `edit_posts`; the target `postId` also requires `edit_post` for that post                                    | Feature-gated; exposed on the dedicated MCP server only                                                                                                                                                                                                                                                                                                                                                       | Queues a review-gated post-blocks structural apply (≤3 path-addressed bounded operations against one post or page's `post_content`, lock-aware) from a `recommend-post-blocks` result and re-checks freshness at request; mutates nothing until a site admin approves it in `Settings > AI Activity`                        | No first-party editor UI; external-agent surface                     |
| `flavor-agent/request-template-apply`  | `edit_theme_options`                                                                                              | Feature-gated; exposed on the dedicated MCP server only                                                                                                                                                                                                                                                                                                                                                       | Queues a review-gated page-level template structural apply (one bounded `insert_pattern` against one `wp_template`) from a `recommend-template` result, replaying the same signed request envelope for freshness checks when optional `visiblePatternNames`, `designSemantics`, `editorSlots`, or `editorStructure` were part of the recommendation; mutates nothing until a site admin approves it in `Settings > AI Activity` | No first-party editor UI; external-agent surface                     |
...
| `flavor-agent/undo-activity`           | Contextual per row (style, template, and template-part: `edit_theme_options`; post-blocks: `edit_post`)          | Feature-gated; dedicated MCP server only                                                                                                                                                                                                                                                                                                                                                                      | Server-side ordered undo with drift checks for executed style, template, template-part, and post-blocks rows (reversed through the executor registry)                                                                                                                                                                                                                           | No first-party editor UI; external-agent surface                     |

## Ability Notes

- `inc/Abilities/Registration.php` defines 35 ability contracts. The 14 helper/read abilities register during `wp_abilities_api_init` when `wp_register_ability()` is available; the `flavor-agent` category registers separately during `wp_abilities_api_categories_init` when `wp_register_ability_category()` is available. The six `preview-recommend-*` siblings register on the same always-on path but are gated on `FeatureBootstrap::canonical_contracts_available()` (Abilities API plus the WP AI plugin's `Abstract_Ability` contract) because they extend that abstract class. The eight AI recommendation abilities (`recommend-block`, `recommend-content`, `recommend-patterns`, `recommend-template`, `recommend-template-part`, `recommend-post-blocks`, `recommend-navigation`, and `recommend-style`) and the seven external-apply abilities (`request-style-apply`, `request-template-apply`, `request-template-part-apply`, `request-post-blocks-apply`, `get-activity`, `list-activity`, and `undo-activity`) register only when the WordPress AI plugin feature contracts are available and the Flavor Agent AI feature is enabled in `Settings > AI`. The preview siblings are deliberately available BEFORE the feature gate is flipped so operators can use the Abilities Explorer to verify wiring without first enabling recommendations.
...
- The six `preview-recommend-*` siblings and ten externally-useful read helpers (`introspect-block`, `list-allowed-blocks`, `list-patterns`, `get-pattern`, `list-template-parts`, `list-templates`, `get-active-theme`, `get-theme-presets`, `get-theme-styles`, `get-theme-tokens`) declare `meta.mcp.public = true` so the universal MCP default server (when mcp-adapter is installed) surfaces them through its `discover-abilities` / `execute-ability` flow. The eight write-side recommendation abilities are deliberately **excluded** from `meta.mcp.public` — they are curated onto the dedicated server only (next bullet), with the read-only preview siblings standing in as the universal server's recommend-surface dry-run. Three helpers stay Abilities-API-only: `list-synced-patterns` and `get-synced-pattern` (can return draft `wp_block` content), and `check-status` (exposes backend-config inventory).
- When the MCP Adapter is active and the recommendation feature is enabled, Flavor Agent also registers a dedicated server at `/wp-json/mcp/flavor-agent`. The dedicated server exposes the eight recommendation abilities, the seven external-apply abilities (`request-style-apply`, `request-template-apply`, `request-template-part-apply`, `request-post-blocks-apply`, `get-activity`, `list-activity`, and `undo-activity`), and four read-only discovery helpers (`list-templates`, `list-template-parts`, `list-patterns`, `get-theme-styles`) as first-class MCP tools — nineteen in all — so external agents see them directly in `tools/list`; the preview siblings stay off the dedicated server (operator preflight is via the universal default bridge); the universal MCP default server remains available for generic ability discovery and execution.
- The four discovery helpers are there because the dedicated server is a **separate endpoint**: a client configures one URL and sees only that server's `tools/list`. Without them, twelve of the fifteen curated tools are advertised but uncallable, because their required inputs are refs a client cannot enumerate — `templateRef` (`recommend-template`, `request-template-apply`), `templatePartRef` / `templatePartId` (`recommend-template-part`, `request-template-part-apply`), `scope` + `styleContext` (`recommend-style`, `request-style-apply`), the `global_styles:<id>` `scopeKey` (`list-activity`), and `visiblePatternNames` (`recommend-patterns`). Two more are enforced in the execute callback rather than the schema, so they do not appear in any `required` array. `recommend-patterns` without `visiblePatternNames` returns an empty recommendation set as a **successful** response — a silent dead end rather than an error, which is why `list-patterns` is on the server. `recommend-navigation` returns `missing_navigation_input` unless `menuId` or `navigationMarkup` is present, and **nothing on this server reliably supplies either**: `list-template-parts` returns `wp_template_part` records, not `wp_navigation` posts. It yields usable markup only when a template part happens to embed a Navigation block *and* the caller holds `edit_theme_options` for `includeContent` — a coincidence, not a contract, and an earlier version of this document recorded it as a supplier on that basis. `MCPServerDiscoveryClosureTest` now classifies it runtime-unresolved. The test enforces both classes: every schema-required input is either produced by another tool on this server, suppliable cold, echoed from an earlier response, or an explicitly recorded exception; and every runtime-enforced input either has a supplier on the same server or is recorded as unresolved.
- This is an **ergonomics** change, not a capability change. All four are already `show_in_rest` and `meta.mcp.public`, so the same principal reaches the same data today over the co-present universal default server — whose transport gate (`is_user_logged_in()`) is looser than this server's `edit_posts || edit_theme_options`. What it removes is a second MCP session and an `execute-ability` round trip in the hot path of a governed apply. It is load-bearing only where a site disables the universal default server via `mcp_adapter_create_default_server`. The `includeContent` downgrade on `list-templates` / `list-template-parts` runs in the execute callback, so template markup stays withheld from callers without `edit_theme_options` regardless of transport admission. Three inputs remain uncallable from a cold start. `recommend-post-blocks.postId` and `request-post-blocks-apply.scope` need a post ID, and Flavor Agent registers no post-listing ability, so a client learns one from core. `recommend-navigation` needs a navigation menu id or markup, and nothing here enumerates `wp_navigation` posts — the lane works for a caller that already holds markup (the editor, or a conversation that supplied it) but cannot be bootstrapped from this server alone.
...
- All 35 defined abilities declare behavior annotations. The eight AI recommendation abilities keep WP-format `meta.annotations.readonly` unset so core and `@wordpress/core-abilities` run calls stay POST for large prompt/editor payloads; they declare `destructive:false`, `idempotent:false`, and `openWorld:true` (LLM provider and docs-grounding backends are external services), and they do not claim direct MCP `readOnlyHint:true` because execution can persist request diagnostics and freshness tokens. The 13 data-read abilities declare WP-format `readonly:true`, `destructive:false`, and `idempotent:true`; `openWorld` stays unset because they read local WordPress registry, theme, and entity state. The six `preview-recommend-*` siblings declare the same `readonly:true, destructive:false, idempotent:true` matrix as the read helpers because they force `resolveSignatureOnly:true` server-side and strip `clientRequest`, so execution never persists diagnostics or freshness tokens — equal inputs always yield equal signature outputs. `flavor-agent/search-wordpress-docs` requires `manage_options` and is not advertised as read-only because direct searches write the shared docs query cache; it declares `openWorld:true` because the docs-grounding backend is an external Cloudflare AI Search call. The seven external-apply abilities split by mutation profile: `get-activity` and `list-activity` declare `readonly:true, destructive:false, idempotent:true` because they only read activity rows; `request-style-apply`, `request-template-apply`, `request-template-part-apply`, and `request-post-blocks-apply` each queue a write and declare `destructive:false, idempotent:false` with `readonly` unset; `undo-activity` declares `destructive:true, idempotent:false` because it reverses an executed style, template, template-part, or post-blocks apply.
- `flavor-agent/request-template-apply` reuses the same governed external-apply loop as `request-style-apply`: it queues a drift-checked **pending** activity row from a `recommend-template` result, a site admin approves it via `POST /flavor-agent/v1/activity/{id}/decision` in `Settings > AI Activity`, and the server then executes one bounded `insert_pattern` against one `wp_template` atomically with re-validation and freshness/drift gates. Gate 1 replays the same signed `recommend-template` envelope when optional `visiblePatternNames`, `designSemantics`, `editorSlots`, or `editorStructure` were part of the recommendation, so editor-informed template recommendations can round-trip through the external approval lane without false stale failures. Executed rows are reversible through `undo-activity`, which dispatches the reverse through the same executor registry that owns the style and template-part lanes. Successful approval and undo emit Ring III attestations in the `external-template-apply-v1` lane for subjects named `wp_template:{theme}//{slug}`.
- `flavor-agent/request-template-part-apply` reuses the same governed external-apply loop as `request-style-apply`: it queues a drift-checked **pending** activity row from a `recommend-template-part` result, a site admin approves it via `POST /flavor-agent/v1/activity/{id}/decision` in `Settings > AI Activity`, and the server then executes ≤3 path-addressed bounded operations against one `wp_template_part` atomically with re-validation and freshness/drift gates. Executed rows are reversible through `undo-activity`, which dispatches the reverse through the same executor registry that owns the style lane. Successful approval and undo emit Ring III attestations in the `external-template-part-apply-v1` lane for subjects named `wp_template_part:{theme}//{slug}`.
- `flavor-agent/request-post-blocks-apply` extends the same governed loop to a fourth subject type: one post or page's `post_content`. It queues a drift-checked **pending** activity row (scope key `{postType}:{postId}`) from a `recommend-post-blocks` result; a site admin approves it via `POST /flavor-agent/v1/activity/{id}/decision`, gated by `manage_options` plus `edit_post` on the target post (`Activity\Permissions` post-scoped defense-in-depth); the server then re-collects the live document target contract (post-type/status allowlist plus lock-aware target exclusion for `attrs.lock`/`templateLock`), re-validates ≤3 path-addressed bounded operations through the same structural-operation grammar `request-template-part-apply` uses, and executes atomically through `wp_update_post`. Executed rows are reversible through `undo-activity`. Post-blocks external applies remain **not attested** because the public `subject-state` route cannot expose potentially non-public `post_content`; that lane needs a separate conditional-subject-state and ID-only subject design.
- Block, pattern, template, template-part, navigation, Global Styles, and Style Book recommendations include `docsGrounding` (`{ available, sourceTypes, count, contentFingerprint, runtimeFingerprint }`) and `docsGroundingFingerprint` (the content/applicability fingerprint alias). Grounding is best-effort prompt context: when the docs search backend is unreachable or returns nothing, the recommendation still runs with `available: false` and clients may show a single soft notice. Grounding never returns an error or blocks a surface; trust and currency of the corpus are owned by `scripts/update-docs-ai-search.js` at ingestion time. `recommend-content` does not attach docs grounding because it is an editorial writing lane rather than a WordPress capability recommendation.
...
- `flavor-agent/check-status` also reports `agentRuntime` — `present` / `supported` / `version` / `minimumVersion` / `reason` / `missingContract` for the optional Automattic Agents API adapter. It is a sibling of `backends`, not a member of it: an absent or unsupported agent runtime says nothing about whether recommendations are configured. See [agents-api-integration.md](agents-api-integration.md).
...
- `flavor-agent/list-template-parts` returns canonical `id` values that match the `templatePartRef` contract accepted by `flavor-agent/recommend-template-part`.
- `flavor-agent/get-theme-styles` returns both raw `styles` and extracted summaries. `elementStyles.base`, `hover`, and `focus` are color-only objects, while `focusVisible` preserves the full `:focus-visible` object. The same helper now also returns a forwardable Global Styles `scope` plus `styleContext.currentConfig` / `mergedConfig` / `availableVariations` so external callers can bootstrap `recommend-style` and refresh stale `request-style-apply` inputs without private PHP seams.
- Helper permissions are intentionally asymmetric: `get-active-theme`, `get-theme-presets`, and `get-theme-tokens` require `edit_posts`; `get-theme-styles`, `list-template-parts` and `list-templates` allow either editor or theme capability at the boundary. `get-theme-styles` joins that OR gate because it supplies the `scope` and `styleContext` that `recommend-style` and `request-style-apply` declare required, and both are `edit_theme_options`: gating the supplier on `edit_posts` alone left the style lane advertised but uncallable for a role holding `edit_theme_options` without `edit_posts`. It grants no data such a role cannot already read through the Site Editor or core's global-styles route. Both `list-template-parts` and `list-templates` silently coerce `includeContent: true` to metadata-only unless the caller has `edit_theme_options`; the theme-oriented recommendation surfaces remain `edit_theme_options` only.
...
| `POST /flavor-agent/v1/activity/{id}/decision` | `manage_options` AND the row's mutation capability                | `src/admin/activity-log.js` approvals UI       | `FlavorAgent\Apply\PendingApplyDecision::decide()`                                                         | Approves or rejects a pending external apply; `{ decision: approve\|reject, note? }`. Approve re-checks freshness and executes server-side through the lane selected by `ExternalApplyExecutorRegistry` |
| `POST /flavor-agent/v1/activity/{id}/claim` | `manage_options` AND the row's mutation capability (`can_decide_activity_request`) | `src/admin/activity-log.js` approvals UI | `FlavorAgent\Apply\ApplyClaim::claim()` | Acquires an advisory, auto-expiring (5 min) review claim on a pending external apply; never gates a decision; cleared on the committed transition |
| `DELETE /flavor-agent/v1/activity/{id}/claim` | `manage_options` AND the row's mutation capability (`can_decide_activity_request`) | `src/admin/activity-log.js` approvals UI | `FlavorAgent\Apply\ApplyClaim::release()` | Releases the caller-owned advisory review claim acquired via `POST .../claim`; if another user holds the claim, the route is a no-op and returns it unchanged. Advisory only, never gates a decision |
| `GET /flavor-agent/v1/attestations/{id}`   | Public                                                               | AI Activity raw envelope link; external verifier | `AttestationController::get_attestation()`                                                                 | Returns the byte-exact signed statement envelope (`statement_b64`, `signature_b64`, `key_id`) plus decoded convenience fields including revert/supersession ids |
| `GET /flavor-agent/v1/attestations/{id}/verification` | Public                                                | AI Activity verification action                 | `AttestationController::get_verification()`                                                               | Returns the strict site-served verification summary (`outcomes`, `verificationStatus`, `terminalAttestationId`, `chainDepth`, `subjectError`). Convenience UX only; independent verification remains the standalone HTTP verifier |
| `GET /flavor-agent/v1/attestations/{id}/subject-state` | Public                                                   | AI Activity raw subject-state link; external verifier | `AttestationController::get_subject_state()`                                                               | Returns the current canonical subject bytes/digest so a verifier can compare live state with the attested digest                       |
| `GET /flavor-agent/v1/attestations/keys`   | Public                                                               | External verifier                              | `AttestationController::get_keys()`                                                                        | Returns the public JWKS-style key registry, including retired keys needed for historical attestations                                   |
...
- `GET /flavor-agent/v1/activity` exposes the wp-admin audit feed when `global=true` or when `scopeKey` is omitted/empty; global reads still require `manage_options`. It rejects malformed active admin date filters with `400` instead of broadening the query; the wp-admin UI also blocks incomplete or inverted persisted date filters until the filter is completed or reset. Its `filterOptions.operationType` values include effective action groups such as `insert` and `replace` plus read-only diagnostics such as `request-diagnostic`. Global reads can pass `includeReports=1` to receive a top-level `learningReport` built from a bounded newest-first sample (default `rowLimit: 500`) with sanitized counts, rates, aggregate groups, truncation metadata, and representative activity ids; scoped editor reads ignore report requests. It is still intentionally a first audit slice rather than a full observability console: the response includes timeline entries, summary counts, pagination, filter options, optional report data, and before/after state payloads. The first rich visual diff viewer in `Settings > AI Activity` is derived client-side from those existing operations and snapshots rather than from a second REST diff contract, and broader operator workflows remain out of scope.
- Pending external-apply rows carry `entry.apply` (`status`, `requestedBy`, `expiresAt`, `operations`, freshness `signatures`, and decision provenance such as `decidedBy`/`decidedByName` (durable display-name snapshot of the approver, captured at decision time)/`decidedAt`/`decisionNote`), hydrated from the stored `request.apply` payload. The `execution_result` column mirrors the lifecycle for SQL filtering — `pending`/`rejected`/`expired`/`failed` before execution, becoming `applied` on approved execution. Non-executed rows (pending, rejected, expired, approval-failed) never participate in ordered undo and never block undo of older executed rows.
- Public attestation routes are unauthenticated by design. They expose only public-safe governed-change statement data, a key registry, the site-served verification summary used by wp-admin, and the minimal canonical subject state needed to verify whether an attested external style, template, or template-part apply still matches the live site. They do not expose prompts, provider payloads, user display names, raw full config blobs, or general activity history. Every response, including a public error response, uses `Cache-Control: no-store, no-cache, must-revalidate, max-age=0`. Post-blocks remains outside Ring III because its potentially non-public `post_content` cannot use this public subject-state contract.
...
```json
{
  ...
  "docsGrounding": {
    ...
    "count": 1,
    "contentFingerprint": "sha256-of-docs-content",
    "runtimeFingerprint": "sha256-of-docs-runtime"
  },
  "docsGroundingFingerprint": "sha256-of-docs-content",
...
Post-blocks external agent -> flavor-agent/recommend-post-blocks ability -> PostBlocksAbilities -> PostBlocksPrompt -> request-post-blocks-apply -> approve -> PostBlocksApplyExecutor -> undo
...
- Signature-only requests return only the current freshness field(s) and the docs-grounding summary after normalizing the current server context and prompt; they compute docs grounding from local cache state only and skip model calls and live docs searches.
...
- Activity permissions are contextual: post-like scopes use `edit_posts` or `edit_post`, while template, template-part, navigation, Global Styles, and Style Book scopes require `edit_theme_options`. Post-blocks pending/decision rows are post-scoped (`{postType}:{postId}`), so they use `edit_posts`/`edit_post` like other post-like scopes, plus `manage_options` for the decision route.
