...
- Seventeen abilities declare `meta.mcp.public = true` — the six `preview-recommend-*` siblings, `search-wordpress-docs`, and ten externally-useful read helpers (`introspect-block`, `list-allowed-blocks`, `list-patterns`, `get-pattern`, `list-template-parts`, `list-templates`, `get-active-theme`, `get-theme-presets`, `get-theme-styles`, `get-theme-tokens`) — so the universal MCP default server (when mcp-adapter is installed) surfaces them through its `discover-abilities` / `execute-ability` flow. The eight write-side recommendation abilities declare `meta.mcp.public = false`; they are curated onto the dedicated server only (next bullet), with the read-only preview siblings standing in as the universal server's recommend-surface dry-run. Three helpers stay Abilities-API-only, also via an explicit `false`: `list-synced-patterns` and `get-synced-pattern` (can return draft `wp_block` content), and `check-status` (exposes backend-config inventory).
- **The `false` must be explicit.** Since mcp-adapter 0.6.0 (`WP\MCP\Abilities\McpAbilityExposure`, upstream #254) the default server resolves exposure as: a malformed `meta.mcp` fails closed → an explicit `meta.mcp.public` wins → otherwise exposure is **inherited** from the high-level `meta.public` flag, which WordPress 7.1 resolves to a boolean on every ability. Omitting the `mcp` key therefore delegates the decision to `meta.public` rather than defaulting to private. Every Flavor Agent ability declares `meta.mcp.public` explicitly, and `RegistrationTest::test_every_registered_ability_declares_mcp_exposure_explicitly()` fails the build if one stops doing so or if the public surface drifts from the seventeen listed above. Note this resolution applies only to the universal default server and the `discover-abilities` / `get-ability-info` / `execute-ability` helpers; a dedicated server built from an explicit tool list never consults it.
...
- Approval and server-side undo authorize the executor's canonical target before any content read, and identity validation precedes capability: `flavor_agent_apply_target_mismatch` for missing, malformed, non-canonical, or divergent identities, `flavor_agent_apply_target_forbidden` for a coherent target the current user cannot edit. The two lanes surface those codes differently. `undo-activity` propagates the `WP_Error`, so the caller sees HTTP 409 or HTTP 403. `POST /flavor-agent/v1/activity/{id}/decision` does not: an authorization failure terminalizes the activity row instead, so the response is HTTP 200 carrying `apply.status: "failed"` with `apply.failureCode` and `apply.failureMessage` set to that code and message (`PendingApplyDecision::decide_with_claim()` then `Agent_Controller::handle_activity_decision()`). Approval clients must branch on `apply.status`, not on the HTTP status. See `docs/reference/governance-layer.md`.
