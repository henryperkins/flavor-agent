...
- The five executable surfaces store a server `resolvedContextSignature` on normal responses. PHP computes that hash from the server-normalized apply context plus the sanitized prompt, so it captures server-only apply-time context while intentionally excluding docs-guidance cache churn.
- Template, template-part, Global Styles, and Style Book responses also include a server `reviewContextSignature` used for review freshness. That digest is built from bounded prompt-shaper inputs (for example prompt-visible patterns, theme tokens, style-book review manifest fields, and bounded docs-guidance excerpts).
- `flavor-agent/recommend-block`, `flavor-agent/recommend-template`, `flavor-agent/recommend-template-part`, and `flavor-agent/recommend-style` accept an optional boolean `resolveSignatureOnly`. When true, they resolve server freshness signatures without running model calls. Block returns only `resolvedContextSignature`; template/template-part/style return both `reviewContextSignature` and `resolvedContextSignature`.
...
| `POST /flavor-agent/v1/recommend-template` | `edit_theme_options` | `fetchTemplateRecommendations()` | `TemplateAbilities::recommend_template()` | Accepts `resolveSignatureOnly`; normal and signature-only responses include both `reviewContextSignature` and `resolvedContextSignature` |
| `POST /flavor-agent/v1/recommend-template-part` | `edit_theme_options` | `fetchTemplatePartRecommendations()` | `TemplateAbilities::recommend_template_part()` | Accepts `resolveSignatureOnly`; normal and signature-only responses include both `reviewContextSignature` and `resolvedContextSignature` |
| `POST /flavor-agent/v1/recommend-style` | `edit_theme_options` | `fetchGlobalStylesRecommendations()` and `fetchStyleBookRecommendations()` | `StyleAbilities::recommend_style()` | Accepts `resolveSignatureOnly`; normal and signature-only responses include both `reviewContextSignature` and `resolvedContextSignature` |
...
- Normal `recommend-block`, `recommend-template`, `recommend-template-part`, and `recommend-style` responses include `resolvedContextSignature`.
- Template/template-part/style responses also include `reviewContextSignature` on normal and signature-only responses so review freshness can detect server-only prompt-shaper drift.
- Signature-only requests skip model calls. Block keeps the `{ payload, clientId }` envelope and returns only `resolvedContextSignature`; template/template-part/style return top-level `reviewContextSignature` plus `resolvedContextSignature`.
