...
- The four discovery helpers are there because the dedicated server is a **separate endpoint**: a client configures one URL and sees only that server's `tools/list`. Without them, twelve of the fifteen curated tools are advertised but uncallable, because their required inputs are refs a client cannot enumerate — `templateRef` (`recommend-template`, `request-template-apply`), `templatePartRef` / `templatePartId` (`recommend-template-part`, `request-template-part-apply`), `scope` + `styleContext` (`recommend-style`, `request-style-apply`), the `global_styles:<id>` `scopeKey` (`list-activity`), and `visiblePatternNames` (`recommend-patterns`). Two more are enforced in the execute callback rather than the schema, so they do not appear in any `required` array: `recommend-navigation` returns `missing_navigation_input` unless `menuId` or `navigationMarkup` is present (`list-template-parts` with `includeContent` yields markup carrying `<!-- wp:navigation {"ref":N} /-->`), and `recommend-patterns` without `visiblePatternNames` returns an empty recommendation set as a **successful** response — a silent dead end rather than an error, which is why `list-patterns` is on the server. `MCPServerDiscoveryClosureTest` enforces both classes: every schema-required input of every tool on this server is either produced by another tool on it, suppliable cold, echoed from an earlier response, or an explicitly recorded exception; and every runtime-enforced input has a supplier on the same server.
- This is an **ergonomics** change, not a capability change. All four are already `show_in_rest` and `meta.mcp.public`, so the same principal reaches the same data today over the co-present universal default server — whose transport gate (`is_user_logged_in()`) is looser than this server's `edit_posts || edit_theme_options`. What it removes is a second MCP session and an `execute-ability` round trip in the hot path of a governed apply. It is load-bearing only where a site disables the universal default server via `mcp_adapter_create_default_server`. The `includeContent` downgrade on `list-templates` / `list-template-parts` runs in the execute callback, so template markup stays withheld from callers without `edit_theme_options` regardless of transport admission. The two remaining uncallable-from-cold inputs are `recommend-post-blocks.postId` and `request-post-blocks-apply.scope`: Flavor Agent registers no post-listing ability, so a client learns a post ID from core.
