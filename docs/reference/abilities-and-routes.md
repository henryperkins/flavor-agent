...
- When the MCP Adapter is active and the recommendation feature is enabled, Flavor Agent also registers a dedicated server at `/wp-json/mcp/flavor-agent`. The dedicated server exposes the seven recommendation abilities plus the five external-apply abilities (`request-style-apply`, `request-template-part-apply`, `get-activity`, `list-activity`, and `undo-activity`) as first-class MCP tools — twelve in all — so external agents see them directly in `tools/list`; the preview siblings stay off the dedicated server (operator preflight is via the universal default bridge); the universal MCP default server remains available for generic ability discovery and execution.
