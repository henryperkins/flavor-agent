...
- `flavor-agent/request-template-apply` reuses the same governed external-apply loop as `request-style-apply`: it queues a drift-checked **pending** activity row from a `recommend-template` result, a site admin approves it via `POST /flavor-agent/v1/activity/{id}/decision` in `Settings > AI Activity`, and the server then executes one bounded `insert_pattern` against one `wp_template` atomically with re-validation and freshness/drift gates. Gate 1 replays the same signed `recommend-template` envelope when optional `visiblePatternNames`, `designSemantics`, `editorSlots`, or `editorStructure` were part of the recommendation, so editor-informed template recommendations can round-trip through the external approval lane without false stale failures. Executed rows are reversible through `undo-activity`, which dispatches the reverse through the same executor registry that owns the style and template-part lanes. Like the template-part lane, **template external applies are not attested** — Ring III attestation stays frozen to the `external-style-apply-v1` lane.
- `flavor-agent/request-template-part-apply` reuses the same governed external-apply loop as `request-style-apply`: it queues a drift-checked **pending** activity row from a `recommend-template-part` result, a site admin approves it via `POST /flavor-agent/v1/activity/{id}/decision` in `Settings > AI Activity`, and the server then executes ≤3 path-addressed bounded operations against one `wp_template_part` atomically with re-validation and freshness/drift gates. Executed rows are reversible through `undo-activity`, which dispatches the reverse through the same executor registry that owns the style lane. Unlike the style lane, **template-part external applies are not attested** — Ring III attestation stays frozen to the `external-style-apply-v1` lane.
- `flavor-agent/request-post-blocks-apply` extends the same governed loop to a fourth subject type: one post or page's `post_content`. It queues a drift-checked **pending** activity row (scope key `{postType}:{postId}`) from a `recommend-post-blocks` result; a site admin approves it via `POST /flavor-agent/v1/activity/{id}/decision`, gated by `manage_options` plus `edit_post` on the target post (`Activity\Permissions` post-scoped defense-in-depth); the server then re-collects the live document target contract (post-type/status allowlist plus lock-aware target exclusion for `attrs.lock`/`templateLock`), re-validates ≤3 path-addressed bounded operations through the same structural-operation grammar `request-template-part-apply` uses, and executes atomically through `wp_update_post`. Executed rows are reversible through `undo-activity`. Post-blocks external applies are **not attested** either — Ring III attestation stays frozen to the `external-style-apply-v1` lane.
