...
- When the MCP Adapter is active and the recommendation feature is enabled, Flavor Agent also registers a dedicated server at `/wp-json/mcp/flavor-agent`. The dedicated server exposes the eight recommendation abilities plus the seven external-apply abilities (`request-style-apply`, `request-template-apply`, `request-template-part-apply`, `request-post-blocks-apply`, `get-activity`, `list-activity`, and `undo-activity`) as first-class MCP tools — fifteen in all — so external agents see them directly in `tools/list`; the preview siblings stay off the dedicated server (operator preflight is via the universal default bridge); the universal MCP default server remains available for generic ability discovery and execution.
...
| `POST /flavor-agent/v1/activity/{id}/undo` | Contextual editor/theme capability                                    | `undoActivity()`                               | `FlavorAgent\Apply\UndoCoordinator::run_verified_undo()`, falling back to `ActivityRepository::update_undo_status()` | Reverts the live subject through the surface executor, then records the outcome. `{ status?: undone\|failed, error? }` — a caller may report its own failure, but an `undone` claim is never taken on trust |
...
| `GET /flavor-agent/v1/attestations/keys`   | Public                                                               | External verifier                              | `AttestationController::get_keys()`                                                                        | Returns the public JWKS-style key registry, including retired keys needed for historical attestations. Each key's `status` is recomputed at serve time: `active` only for the key the site can sign with right now; a stored-active key whose private half is unavailable is served as `verification_only` (still published, still verifies its historical attestations) |
...
- `POST /flavor-agent/v1/activity/{id}/undo` is the editor's undo report-back channel **and** a governed undo path. Both it and the `undo-activity` ability dispatch through `FlavorAgent\Apply\UndoCoordinator::run_verified_undo()`, so the two cannot diverge. The route applies the same pre-checks the ability does (never-executed guard, `available`-only transition, ordered-undo gate) before any executor runs, then:
  - **Server-verified.** When the row's surface has an executor and the row carries the snapshot that executor compares against, the executor reads live state and reverts it. `undo.verification: "server"`. Live state equal to `before` resolves as `result: "already_undone"` without a second write — the ordinary case, because the editor reverts client-side before reporting. Live state matching neither snapshot writes `undo.status: "failed"` with the drift message and returns `409 flavor_agent_undo_drift`.
  - **Client-reported.** When the surface has no executor (`block`, `navigation`, `content`) or the row records operations rather than a subject snapshot (editor-authored `template` / `template-part` rows), the subject lives in the editor and there is nothing server-side to check. The transition is recorded as the editor's report and labelled `undo.verification: "client-reported"`, never as a verified revert.
  - **Refused.** A row that executed server-side (`apply.executedAt` set) but carries no comparable snapshot returns `409 flavor_agent_undo_unverifiable` and stays `available`. There is no honest terminal state for it, so none is written.
  - A caller may pass `status: "failed"` to report that its own client-side undo failed; failure never claims a revert happened, so it is recorded without dispatching an executor — including on a server-owned subject, where it also lands as `undo.verification: "client-reported"`. Such a row means the change is **still live** and the server never touched it. `status` accepts only `undone` or `failed`. The response carries `result` (`undone` / `already_undone` / `failed` / `client_reported`) alongside `entry`.
  - The ordered-undo rule governs whether an undo may **start**, not whether a completed one is recorded. `UndoCoordinator::run_verified_undo()` writes the terminal status with `$revert_already_performed`, so a newer activity landing on the same entity between the caller's gate and the write cannot reject a revert the executor already performed — that would leave the row `available` while the change is gone.
