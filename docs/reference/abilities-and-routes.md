...
- `POST /flavor-agent/v1/activity/{id}/undo` is the editor's undo report-back channel **and** a governed undo path. Both it and the `undo-activity` ability dispatch through `FlavorAgent\Apply\UndoCoordinator::run_verified_undo()`, so the two cannot diverge. The route applies the same pre-checks the ability does (never-executed guard, `available`-only transition, ordered-undo gate) before any executor runs, then:
  ...
  - A caller may pass `status: "failed"` to report that its own client-side undo failed; failure never claims a revert happened, so it is recorded without dispatching an executor. `status` accepts only `undone` or `failed`. The response carries `result` (`undone` / `already_undone` / `failed` / `client_reported`) alongside `entry`.
