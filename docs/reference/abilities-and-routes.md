...
- Approval and server-side undo authorize the executor's canonical target before any content read. Identity validation precedes capability: `flavor_agent_apply_target_mismatch` (HTTP 409) for missing, malformed, non-canonical, or divergent identities; `flavor_agent_apply_target_forbidden` (HTTP 403) for a coherent target the current user cannot edit. See `docs/reference/governance-layer.md`.
