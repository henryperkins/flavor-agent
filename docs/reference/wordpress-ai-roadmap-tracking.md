...
Compete with `inc/Abilities/Registration.php` (29 defined abilities: seven recommendation, twelve helper/read, one docs search, five preview siblings, and four external-apply abilities, with recommendation/external-apply registration gated by the AI feature), `inc/Abilities/SurfaceCapabilities.php`, the surface gating in `src/utils/capability-flags.js`, and the editor hydration of abilities into `@wordpress/core-abilities`.
