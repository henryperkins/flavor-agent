...
- AI plugin release overlay refreshed: 2026-08-23. [`WordPress/ai` `1.3.0`](https://github.com/WordPress/ai/releases/tag/1.3.0) is the latest shipped release (2026-08-18, milestone #22), superseding `1.2.0` (2026-07-14, milestone #19), `1.1.0` (2026-07-01, milestone #21), and the `1.0.2` patch milestone (2026-06-16). The project-board counts elsewhere in this doc remain the 2026-06-20 snapshot — only the release train was re-verified. `1.2.0` requires no Flavor Agent code change; its historical diff read and reasoning remain in the `1.2.0` overlay below. `1.3.0` prompted one proactive next-major compatibility change — removing the deprecated `AI_Service` / `get_ai_service()` route while preserving generation options through canonical SDK key mapping — and adds watches for Custom Abilities opt-in gating, AI-plugin-owned prompt extension points, public non-transporter request logging, settings/data lifecycle, changed soft model preferences, and temporarily unavailable embeddings; see the `1.3.0` overlay. `WordPress/ai#595` ("Better connector approval matching") shipped in `1.0.1` on 2026-05-26, so Flavor Agent's remaining Connector Approval work is post-ship smoke validation, not waiting for the upstream PR.
...
gh api repos/WordPress/ai/milestones/22
gh api 'repos/WordPress/ai/issues?milestone=22&state=all&per_page=100'
```
...
| Latest shipped | `WordPress/ai` `1.3.0` (2026-08-18, milestone #22) |
| Active / next release | Not re-verified in the 2026-08-23 release-train pass; milestone counts not re-snapshotted since the 2026-06-20 board snapshot |
...
- **Logging.** `includes/Logging/` has **zero changed files** across `1.1.0...1.2.0`. The `wpai_request_logs` schema is unchanged, so the dual-logging contract and `request.ai.requestLogId` in `docs/features/activity-and-audit.md` are intact. It also confirms the symlink source-attribution fix tracked at that doc's line 180 has still not landed upstream, so Flavor Agent's defense-in-depth capture bridge stays load-bearing.
...
AI plugin [`1.3.0`](https://github.com/WordPress/ai/releases/tag/1.3.0) shipped on 2026-08-18 (milestone #22). Its **Custom Abilities** experiment gates the AI plugin's own custom abilities behind one opt-in toggle; treat that as discovery/availability context for core abilities, not as a change to Flavor Agent's 35 locally registered abilities or their existing feature and per-surface gates. [`#770`](https://github.com/WordPress/ai/pull/770) adds per-ability prompt filter helpers. Flavor Agent recommendation classes inherit those helpers, but their custom execution pipeline does not invoke them, so no new filter runs here automatically; this is explicit future integration/design precedent rather than an active hook. [`#905`](https://github.com/WordPress/ai/pull/905) deprecates `AI_Service` and `get_ai_service()` in favor of direct `wp_ai_client_prompt()` use. Flavor Agent's service-first branch was removed in this refresh; the direct builder path now explicitly translates the plugin's sanitized snake_case generation options to the SDK's camelCase `ModelConfig` keys so `max_tokens`, `top_p`, and the other non-identical names are not silently dropped.

[`#913`](https://github.com/WordPress/ai/pull/913) updates the AI plugin's ordered Anthropic/Google/OpenAI soft text-model preferences as well as removing custom request temperatures from its own features. Flavor Agent applies `get_preferred_models_for_text_generation()` to every direct builder before applying its own selection. An exact selected model supersedes that list; a provider-only selection constrains the preference search to that provider, so `1.3.0` may still change the chosen model within it. Actual resolution remains connector-dependent and has not been runtime-validated here.

[`#914`](https://github.com/WordPress/ai/pull/914) adds public `WordPress\AI\log_ai_request()` for explicit MCP-tool and ability records when Request Logging is enabled. It is complementary to, not a replacement for, Flavor Agent's transporter-captured request bridge (`wpai_request_log_context` / `wpai_request_logged`) or plugin-owned docs-grounding diagnostics: those paths need their current context/linking behavior, and docs grounding is not AI Client transport traffic. The opt-in Content Translation experiment (`#747`) can mutate Paragraph/Heading blocks and optionally the title; if co-enabled, Flavor Agent's fresh server context, request signatures, and approval-time drift checks remain the coexistence boundary. `1.3.0` also renames its own `ai_*` metadata keys to `wpai_*` and tightens sanitization of LLM-bound/rendered content. Those are upstream experiment postures, not authorization to weaken Flavor Agent's metadata boundaries, request policy, or sanitization/validation contract.

Settings portability and deletion now include AI-plugin-owned Flavor Agent state. [`#734`](https://github.com/WordPress/ai/pull/734) exports/imports registered non-sensitive settings in the AI plugin option group; while Flavor Agent is active, that includes `wpai_feature_flavor-agent_enabled` and `wpai_feature_flavor-agent_field_developer`, and Site Health counts the enabled feature. It excludes the separate `flavor_agent_*` settings group and its Cloudflare/Qdrant secrets. [`#692`](https://github.com/WordPress/ai/pull/692) removes all `wpai_*` options and drops the core `wpai_request_logs` table when the AI plugin is deleted by default (filterable via `wpai_remove_data_on_uninstall`), but does nothing on deactivation. Deletion therefore resets Flavor Agent's AI-plugin toggle/model selection on reinstall and can leave otherwise valid local activity rows pointing to a core request-log ID whose row no longer exists; Flavor Agent-owned activity and settings are not deleted.

Although `1.3.0` carries embedding code and helper functions, [`#946`](https://github.com/WordPress/ai/pull/946) disables `SDK_Overlay` loading while the PHP AI Client integration changes. On supported WordPress 7.0/7.1 environments, `generate_embeddings()` therefore returns the unavailable-environment `WP_Error`; it is not an adoptable core embedding primitive. Flavor Agent must retain its plugin-owned Cloudflare Workers AI embeddings, Qdrant, and retrieval stack until a supported, stable upstream embedding path is available.

Honesty bound: this `1.3.0` overlay records a static upstream release/source diff and local source/test review as of 2026-08-23. The Docker-backed WordPress 7.0 E2E harness passes with AI plugin `1.3.0` installed and active (30/30), but no connector-backed live text-generation request or model-resolution smoke was run.
...
| [`WordPress/ai` milestone #22](https://github.com/WordPress/ai/milestone/22) | `1.3.0` | Released 2026-08-18 | Custom Abilities is opt-in for the AI plugin's own custom abilities; built-in-ability prompt extension points (`#770`) are design precedent only; soft model preferences changed (`#913`); the deprecated `AI_Service` / `get_ai_service()` route (`#905`) was removed locally in favor of direct `wp_ai_client_prompt()` plus canonical `ModelConfig` key mapping; public `log_ai_request()` (`#914`) complements the existing bridge/local diagnostics; and settings import/export plus uninstall cleanup (`#734`, `#692`) now cover AI-plugin-owned Flavor Agent state. Embedding helpers ship but `SDK_Overlay` loading is disabled by `#946`, so `generate_embeddings()` is unavailable on supported WP 7.0/7.1 and Flavor Agent retains plugin-owned embeddings. |
...
1. **Review and smoke test release-train changes before building parallel local features.** Examples: locale-aware word/character counting, DataViews translations, connector enable/disable, settings/data lifecycle, Type Ahead, C2PA Monitor, and alt-text URL matching. Uninstall cleanup shipped in `1.3.0`; its Flavor Agent posture is recorded in the overlay above.
...
| -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `WordPress/ai#21` — How to best support hundreds or thousands of abilities                         | `inc/Abilities/Registration.php` defines 35 abilities (8 recommendation + 13 helper + 1 docs search + 6 preview siblings + 7 external-apply) | Ability consolidation or per-surface registration is now a Future Release governance concern, not a 1.0-era release blocker.                                      |
...
| `WordPress/ai#437` — Request Logging                                                              | `inc/Activity/Repository.php`, `inc/Activity/Serializer.php`, `src/admin/activity-log.js` | Core AI request logging now exists. Coexistence (not consolidation): enrich `wpai_request_logs.context` via the `wpai_request_log_context` filter, dual-log `request_diagnostic` rows alongside core by default (opt-out via AI Activity Dual Logging), keep apply/undo rows local — see `docs/features/activity-and-audit.md`. |
...
| `WordPress/ai#452` / [`#633`](https://github.com/WordPress/ai/pull/633) — Content Classification relevance | `inc/Abilities/ContentAbilities.php`, content panel taxonomy suggestions | Shipped in `1.3.0` as an adjacent taxonomy-classification improvement with no schema or UI change; it does not replace Flavor Agent's recommendation surface. |
...
| ----------------- | ------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `WordPress/ai#21` | How to best support hundreds or thousands of abilities  | In discussion | `inc/Abilities/Registration.php` defines 35 abilities under the `flavor-agent/` namespace (8 recommendation + 13 helper + 1 docs search + 6 preview siblings + 7 external-apply). Helper/read abilities and preview siblings register whenever the Abilities API and AI plugin contracts exist; recommendation and external-apply abilities also require the WordPress AI feature gate. Ten externally-useful read helpers, `search-wordpress-docs`, and all six preview siblings are MCP-public on the universal default bridge; three editor-internal helpers stay Abilities-API-only. The issue argues 1-to-1 ability->tool exposure breaks model tool selection at scale and proposes a "Layered Tool Pattern". Likely outcome: Flavor Agent consolidates into a smaller "router" ability surface, or registers abilities only on opt-in surfaces under issue 354. Per the [2026-05-20 AI contributor summary](https://make.wordpress.org/ai/2026/05/21/ai-contributor-weekly-summary-20-may-2026/), `@justlevine` is the named owner; the action item is to document emerging ecosystem trends and, if necessary, open an expansive Trac ticket for broader core developer feedback. The "find abilities" discovery workflow inside the AI companion plugin is the team's named next technical milestone, mirroring the third-party "find tool" pattern. |
...
| [`WordPress/ai#914`](https://github.com/WordPress/ai/pull/914) | Public `log_ai_request()` for MCP-tool and ability records | Shipped; `1.3.0` | Showcase Plugin |
...
| `WordPress/ai#507` / [`#861`](https://github.com/WordPress/ai/pull/861) | Editorial Updates links to Visual Revisions | Shipped in `1.3.0`; review evidence |
...
| `WordPress/ai#191` / [`#734`](https://github.com/WordPress/ai/pull/734) | Add import/export support for AI settings and provider configuration | Shipped; `1.3.0` |
...
Compete with `inc/Abilities/Registration.php` (35 defined abilities: eight recommendation, thirteen helper/read, one docs search, six preview siblings, and seven external-apply abilities, with recommendation/external-apply registration gated by the AI feature), `inc/Abilities/SurfaceCapabilities.php`, the surface gating in `src/utils/capability-flags.js`, and the editor hydration of abilities into `@wordpress/core-abilities`.
...
| `WordPress/ai#881` | Custom Abilities experiment gate for the AI plugin's own custom abilities                    | Shipped; `1.3.0` |
...
| `WordPress/ai#452` / [`#633`](https://github.com/WordPress/ai/pull/633) | Content Classification: improve relevance of taxonomy suggestions | Shipped; `1.3.0` |
...
| `WordPress/ai#187` / [`#747`](https://github.com/WordPress/ai/pull/747) | Content Translation experiment; opt-in Paragraph/Heading and optional title mutation | Shipped; `1.3.0`; coexist through Flavor Agent freshness/drift checks |
| `WordPress/ai#188` | Add persona-driven content generation experiments                           | Backlog       |
| `WordPress/ai#192` / [`#770`](https://github.com/WordPress/ai/pull/770) | Add extension points for custom prompt templates | Shipped; `1.3.0` |
...
If core ships a native embeddings + vector-search primitive (`#683`), revisit whether Flavor Agent's Cloudflare Workers AI embeddings, Qdrant storage, and pattern-retrieval backends should defer to it the way chat now defers to Connectors. Treat it as a future provider-ownership migration candidate, not current work: `#683` is a Future Release draft and core embeddings are not yet exposed through Connectors. **Currentness check 2026-07-18:** the library layer moved first — `php-ai-client` `1.4.0` (tagged 2026-07-15) shipped the embedding-generation APIs (`WordPress/php-ai-client#244`) that the WordPress-side integration consumes, while the WordPress-core embeddings integration missed the 7.1 Beta 1 cutoff (the team's pencils-down deadline was 2026-07-14) and is now targeted at WordPress 7.2, with an open question about attempting 7.1 Beta 2 instead, per the [2026-07-15 AI contributor summary](https://make.wordpress.org/ai/2026/07/17/ai-contributor-weekly-summary-15-july-2026/). **Currentness correction 2026-08-23:** AI plugin `1.3.0` includes embedding helpers, but [`#946`](https://github.com/WordPress/ai/pull/946) disables `SDK_Overlay`; on supported WordPress 7.0/7.1 the helper returns an unavailable-environment error. The migration posture therefore remains watch-only.

### MCP And WebMCP

Flavor Agent exposes abilities over MCP through two servers, both of which require the `mcp-adapter` plugin. The **universal default server** resolves exposure through `WP\MCP\Abilities\McpAbilityExposure` (mcp-adapter 0.6.0+, upstream #254): a malformed `meta.mcp` fails closed, an explicit `meta.mcp.public` wins, and otherwise exposure is **inherited** from the high-level `meta.public` flag that WordPress 7.1 resolves on every ability. It surfaces the 10 read helpers, `search-wordpress-docs`, and the 6 read-only `preview-recommend-*` siblings (17 tools) via `discover-abilities` / `execute-ability`. The 8 write-side `recommend-*` abilities and the 7 external-apply abilities declare `meta.mcp.public = false` **explicitly** (`Registration::recommendation_meta()` / `external_apply_meta()`) rather than omitting the key, because under the inheritance rule an absent key would delegate the decision to `meta.public`: `request_diagnostic` rows can carry prompts, so generic discover/execute exposure stays curated. They are instead published on a **dedicated Flavor Agent MCP server** at `/wp-json/mcp/flavor-agent` (`inc/MCP/ServerBootstrap.php`) whose explicit tool list is the 8 recommendation + 7 external-apply abilities (15 tools in `tools/list`); it registers only when the recommendation feature is enabled, and transport access requires `edit_posts`/`edit_theme_options`. The 3 editor-internal helpers (`list-synced-patterns`, `get-synced-pattern`, `check-status`) are on neither MCP server — Abilities-API-only, also via an explicit `meta.mcp.public = false`. Future upstream MCP routing work still pressures provider and ability-routing layers, but Flavor Agent no longer relies only on the universal default-server bridge.
...
| #                  | Title                                              | Status      |
| ------------------ | -------------------------------------------------- | ----------- |
| `WordPress/ai#233` | Refactor experiments to leverage AI_Service layer  | June 20 board snapshot; superseded by `1.3.0` deprecation `#905` |
...
1. ~~**`#437`, `#419`** — Request Logging shipped in AI `1.0.0`; subscribe and mirror future Site Agent review. The design is **coexistence, not consolidation**: core's Request Logging captures every AI Client HTTP call transparently via the SDK HTTP transporter decorator, so Flavor Agent should enrich `wpai_request_logs.context` with surface/scope/document/ability data via the `wpai_request_log_context` filter, stop persisting `request_diagnostic` rows when core logging is enabled, and keep the apply/undo journal plus the editor-inline `AIActivitySection` history in `inc/Activity/Repository.php`. Full design at `docs/features/activity-and-audit.md`.~~ **Done 2026-05-25 via Request Logging bridge, revised 2026-06-03 for dual logging: `RequestLoggingBridge` injects context, captures `wpai_request_logged` IDs, keeps Flavor Agent diagnostics by default, and the Activity admin can inspect the matching core row inline.** Suppression is now the AI Activity Dual Logging opt-out. [`#914`](https://github.com/WordPress/ai/pull/914) adds a public explicit-record API in `1.3.0`; it is complementary for future non-transporter operations, not a replacement for this bridge or local docs diagnostics.
2. **`#21`, `#354`, Custom Abilities `1.3.0`** — Plan ability consolidation. Decide whether the 35 defined abilities under `flavor-agent/` collapse into a smaller router surface, or remain individual with helper/read and preview abilities always available and recommendation/external-apply abilities registered against per-surface gates once `#354` defines them. The AI plugin's Custom Abilities experiment only gates its own custom core abilities and does not replace Flavor Agent's gates. **Tracked as an upstream watch item in `docs/reference/current-open-work.md`; not yet an implementation plan.**
...
7. ~~**`#345`, `#437`, `#419`** — Define the activity story now that Request Logging and usage-safeguards work shipped in the AI plugin. The decision lands on coexistence (see implication 1 above and `docs/features/activity-and-audit.md`): core Request Logging owns provider/model/tokens/cost observability; Flavor Agent's Activity Repository owns apply/undo state; the admin audit page cross-links into `Tools → AI Request Logs` rather than duplicating or retiring it. Cost and limit metering vocabulary should align with core's safeguards rather than inventing a parallel metering layer.~~ **Done 2026-05-25 via Request Logging bridge Phase 1-4; cost stays in core Request Logs while Flavor Agent shows linked request details and local apply/undo state.**
8. **`#192`, [`#770`](https://github.com/WordPress/ai/pull/770)** — `1.3.0` shipped per-ability prompt filter helpers. Flavor Agent recommendation classes inherit them, but their custom execution pipeline does not invoke them, so no new filter runs automatically. Treat explicit adoption as future design work, and keep Flavor Agent-specific extension points out of scope until a concrete consumer and security boundary exist. **Not yet covered by an existing workstream.**
...
14. **`WordPress/ai#683` (native vector search / RAG), [`#946`](https://github.com/WordPress/ai/pull/946)** — Watch core's MariaDB-backed embeddings/vector-search experiment as a future provider-ownership migration point for Flavor Agent's plugin-owned embedding (Cloudflare Workers AI), Qdrant storage, and pattern-retrieval backends. Do not migrate now: `#683` is a Future Release draft and core embeddings are not exposed through Connectors. `1.3.0` carries helpers but disables `SDK_Overlay`, so `generate_embeddings()` returns unavailable-environment `WP_Error` on supported WP 7.0/7.1; it is not an available migration target. **Not yet covered by an existing workstream.**
