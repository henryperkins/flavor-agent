...
Latest Gutenberg release at snapshot: `23.9.0` (published 2026-09-02). Flavor Agent supports the editor bundled with its public WordPress `7.0+` support range and separately tracks compatibility with the latest Gutenberg plugin release. Its direct `@wordpress/*` dependencies, Playground harness, and default Docker target align to the package/plugin versions published from the Gutenberg `v23.9.0` tag. The required Docker CI matrix covers both the editor bundled with WordPress `7.1` and exact Gutenberg `23.9.0`. Explicit older Gutenberg plugin versions remain available for diagnosis, not as the current release target.

Local browser evidence (2026-09-12): the uncommitted save-persistence implementation on base `da478deb343e3d0ef2008fafab1a7ace258592ae` passed Playground 17/17 and both Docker editor legs 34/34. Docker ran WordPress `7.1` / PHP `8.2.33`; WP-CLI confirmed active Gutenberg `23.9.0` for its plugin leg and inactive Gutenberg before and after the bundled-editor leg. The tests include real save, editor Undo, subsequent save, lost-response, and block-identity coverage. The [verification record](../validation/2026-09-12-save-persistence-outcomes.md) preserves source/build hashes, per-step logs, and the excluded first bundled attempt whose reused environment still had Gutenberg active. This updates local execution evidence; the upstream roadmap snapshot above remains dated September 4.

September 4 browser evidence: the WordPress 7.1 Playground harness installed and activated the exact `gutenberg.23.9.0.zip` package through its checked-in blueprint and passed 17/17. The isolated Docker Site Editor harness then passed 32/32 with WordPress `7.1` plus Gutenberg `23.9.0` after verifying both versions from inside the runtime, and passed 32/32 with WordPress `7.1` after verifying that Gutenberg was not installed. Docker used the documented public `mariadb:11.4` fallback because the default DHI registry returned HTTP 401 before WordPress started; the database-image fallback does not change the pinned WordPress or selected editor runtime.

Historical browser evidence: Gutenberg `23.7.1` was green in CI run `31468196111` (PR #78, commit `6c79fe0`) on the then-pinned `wordpress:7.0.0-php8.2-apache` image: the Gutenberg leg passed 30/30 and the bundled leg passed 30/30. That closes the recorded 23.6/23.7 audit only; it is not the current support boundary.
Prior real-browser smoke: Gutenberg `23.5.0` on local WordPress `7.1-alpha-62619` passed Flavor Agent surface mount/overflow checks for AI Activity, Global Styles, Style Book, templates, and template parts, with one representative upstream/runtime observation: Gutenberg emits the React 19 `inert` boolean-attribute console warning. A prior `/wp/v2/registered-templates` 500 came from an accidental non-baseline AI Services activation; after removing `ai-services`, the route returns 200 while core/Gutenberg still emits null-post-type capability warnings.
...
| 23.5 | 2026-07-01 | Media Editor cropping and Cover block crop support; unified device preview with resizable editor canvas; Global Styles `textShadow`; Gutenberg minimum WP version becomes 6.9; `@wordpress/theme` publicly exports `ThemeProvider`, provides token defaults without a runtime provider, and renames public `--wpds-color-bg-*` / `--wpds-color-fg-*` tokens to `--wpds-color-background-*` / `--wpds-color-foreground-*`; DataViews view-config REST work moves toward core readiness; Block Fields fixes a pattern-overrides binding crash; block Inspector text and background color controls relocate out of the Color panel into Typography and Background with element colors moving to a new `elements` inspector group (#77279 — filed under Tools in the release notes, which is why it was missed in the first 23.5 pass). |
| 23.6 | 2026-07-22 | Post Editor is now **always iframed**, matching the Site Editor; admin bar shows by default in both editors; Playlist and Tabs blocks graduate from the block-experiments flag; inline notes anchor to a text selection and gain `@mention` autocomplete; custom icon registration API plus a Collections tab in the icon picker; Dynamic Gallery variation; customizable `theme.json` viewport sizes; selective "Apply Globally" for block styles; hard deprecation of the 40px default control size; React 19 peer-dependency support. `@wordpress/core-abilities` restores the public API removed in #78316 (#79155): the module now auto-initializes on import and exports a `ready` promise, and its laziness moves into the module graph — the workflow palette dynamically imports it on open. A static import or script-module dependency therefore fires its `per_page: -1` category and ability fetches on every editor load, which the REST controller rejects. Prefer `23.6.2` (2026-07-28) over `23.6.0` if pinning this line. |
| 23.7 | 2026-08-05 | `@wordpress/theme` **disallows `src` in the published package** (#80213) and removes the private `Theme` component; the public `./design-tokens.css` export moves from `src/prebuilt/css/` to `prebuilt/css/`, and 1.1.0 additionally lists that file in the package `sideEffects` array. Global Styles extracts shared preset management from shadows and font sizes and reflects inherited values in block Inspector controls (behind a Gutenberg experiment). DataViews gains Shift+Click range selection and fixed-width search fields. Components add Combobox/Select `Group` + `GroupLabel` and an `Autocomplete.Row` primitive. Superseded by `23.7.1` (2026-08-07), a security release that addresses the phpcs and wpcs GHSAs (#81279) — the same advisories tracked in Flavor Agent's own `composer.lock`. |
| 23.8 | 2026-08-19 | The empty canvas renders a real default block instead of an appender lookalike, and new sibling blocks consistently inherit neighboring styles. List View substantially reduces rendering and selection work on large posts. Visual revisions gain shareable URLs and a code diff view. `@wordpress/element` is importable under React 19; DataViews adds a time field and configurable preview aspect ratios. |
| 23.9 | 2026-09-02 | The block toolbar exposes insertion for selections where the inserter was hard to reach. Global Styles identifies and filters blocks with custom overrides, adds custom duotone colors, and exposes Citation, Input, and Select element styling. Group flex/grid layouts gain independent horizontal and vertical gaps. Components and DataViews continue CSS-module and `@wordpress/ui` migrations, including the `ComboboxControl` `onChange` narrowing to `string | null`. |
...
| Global Styles `textShadow` | **Shipped 2026-07-20.** `typography.textShadow` is offered on the global-styles surface only — #73320 adds no `block.json` support, so the block scope could never gate on it and `src/store/update-helpers.js` is deliberately unchanged. Gated at runtime by `FlavorAgent\Support\ThemeJsonCapabilities`, which probes the resolved theme.json class rather than comparing versions (Gutenberg's `WP_Theme_JSON_Gutenberg` shadows core's, so `$wp_version` cannot answer). The probe also refuses the path for users without `unfiltered_html`: `text-shadow` is absent from `safe_style_css` on every WordPress line including trunk, so the save-time kses pass would strip the value and produce an apply that silently does nothing. Values are validated structurally by a matched PHP/JS validator pair. | `vendor/bin/phpunit --filter 'StyleAbilities\|StylePrompt\|ThemeJsonCapabilities'`; `npm run test:unit -- --runTestsByPath src/utils/__tests__/style-validation.test.js src/context/__tests__/theme-tokens.test.js --runInBand`; on a real 23.5 runtime confirm a text-shadow apply persists for an `unfiltered_html` user and is refused otherwise. |
| Block Inspector color control relocation (#77279) | **Shipped 2026-07-20.** Flavor Agent no longer delegates a `color` chip group. Core removed its Fill from `hooks/color.js` while the `color` Slot remains, so the plugin's fill would have been the only one in that slot and would have resurrected an otherwise-absent "Color" panel holding an AI chip and no color control. Color suggestions stay visible and appliable in the main `AI Recommendations` panel, which never filtered by panel. Version-gating was rejected: the chips are passive mirrors, so removal costs a convenience rather than a capability. | `npm run test:unit -- --runTestsByPath src/inspector/__tests__/panel-delegation.test.js src/inspector/__tests__/InspectorInjector.test.js --runInBand`; on 23.5 confirm no empty Color panel appears for a block whose only color supports are text/background. |
