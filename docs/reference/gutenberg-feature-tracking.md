...
Latest Gutenberg release at snapshot: `23.7.1` (published 2026-08-07, a security release two days after `23.7.0`; it is the browser target for the 23.7 line, not `23.7.0`). Static alignment for 23.6/23.7 landed three changes: the abilities bridge no longer loads `@wordpress/core-abilities`, `wpds-runtime.css` imports design tokens through the `@wordpress/theme` public export, and the WP 7.0 harness can pin a Gutenberg version. Real-browser result on 23.7.1: **green**. CI run `31468196111` (PR #78, commit `6c79fe0`) provisioned `wordpress:7.0.0-php8.2-apache` with `Companion plugins: ai, gutenberg@23.7.1` and the WP 7.0 Site Editor suite reported `30 passed (5.1m)` — covering the Site Editor template, template-part, Global Styles, and Style Book surfaces, AI Activity, external-apply approvals, and the block Inspector smoke. The `bundled` leg (same image, no Gutenberg) passed `30 passed (4.4m)` in the same run, so the two editors agree. This exercised 23.6's always-iframed post editor and admin-bar default, the 23.7 Global Styles/DataViews changes, and React 19, none of which regressed a Flavor Agent surface. Note the job is `continue-on-error: true`, so this conclusion comes from the job log, not the check badge. Not yet covered: an explicit `23.6.2` run — 23.7.1 supersedes it, and `--with-gutenberg=23.6.2` is the way to check that line if a 23.6-specific question comes up.
