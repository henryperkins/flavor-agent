...
| Client-side Abilities API (`@wordpress/abilities`, `@wordpress/core-abilities`) | WP 7.0 | `inc/Abilities/Registration.php` | Server abilities hydrate into the client `core/abilities` store automatically. All 20 abilities populate `meta.annotations`; the WP-format `readonly` policy (kept unset for the 7 recommendation abilities to preserve POST routing) and equivalent MCP `readOnlyHint`/`destructiveHint`/`idempotentHint` hints are documented in [`abilities-and-routes.md`](abilities-and-routes.md#ability-notes). |
