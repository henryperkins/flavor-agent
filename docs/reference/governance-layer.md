...
Approval and server-side undo authorize the **canonical WordPress target the executor will read, mutate, and attest**, not the caller's claimed document fields. `ExternalApplyExecutor::resolve_target_identity()` is the single parser for that target; `authorize_target()` runs identity validation before capability, and neither baseline collection nor content reads happen until authorization succeeds. Divergent, missing, malformed, or non-canonical identities fail closed with `flavor_agent_apply_target_mismatch`; a coherent target the current user cannot edit fails with `flavor_agent_apply_target_forbidden`. Both fail closed, but they are **not surfaced the same way**: server-side undo propagates the `WP_Error`, so the caller sees HTTP 409 / HTTP 403, while approval terminalizes the row instead — `PendingApplyDecision::decide_with_claim()` writes `apply.status = "failed"` plus `apply.failureCode`, and the decision route answers HTTP 200. Treat the failure code, not the transport status, as the authorization signal. There is no production filter or caller-controlled authorization seam on this path.
