...
>
> Currentness note, 2026-08-15: the MCP Adapter distribution framing below is superseded. Upstream `WordPress/mcp-adapter` v0.6.1 (2026-08-13) **inverted** the install guidance this audit recorded — the README now recommends the WordPress-plugin form from the GitHub Releases ZIP and demotes Composer to a plugin-developer library path (`78b7373` / #204, `1b96b15` / #223), and upstream removed its git-clone development-version section. A WP.org-format `readme.txt` was added for Plugin Check compliance (`d8812f1` / #187), but there is still no live `wordpress.org/plugins` listing. So "the README has not been updated" is no longer true, while "no WP.org listing" still is. Default-server ability exposure also changed in 0.6.0 (`McpAbilityExposure`, #254): an absent `meta.mcp` key now inherits `meta.public` instead of defaulting to private. Current state lives in `CLAUDE.md`, `docs/reference/local-environment-setup.md`, and `docs/reference/abilities-and-routes.md`. The line references below (`CLAUDE.md` line 48, `local-environment-setup.md` lines 83 and 264) also predate later edits and no longer resolve.
>
