...
```bash
wp eval-file scripts/demo-agents-api.php --user=1                       # tiers A + C: free, deterministic
FA_DEMO_TIERS=A,B,C wp eval-file scripts/demo-agents-api.php --user=1   # adds a real provider call
```

`--user` is required rather than decorative: WP-CLI runs with no acting user by default, so capability checks resolve against user 0 and both the `edit_posts`-gated `check-status` read and the administrator-gated `agents/chat` call are denied. Check `A0` fails loudly when it is missing, so that misconfiguration cannot be mistaken for a wiring failure.
...
- **Tier B — live run.** A real `agents/chat` turn. Evidence that the runtime dispatched Flavor Agent abilities comes from upstream's own `metadata.agents_api.tool_observability.calls[]`, not from parsing the reply; the run id is then correlated against activity rows. B2 requires a `recommend-*` call whose reported `status` is `succeeded`, not merely a dispatched one — a recommendation that fails on schema, permission or provider error still appears in that record and still writes a failure diagnostic carrying the run id, so counting attempts would let a turn in which every recommendation errored satisfy the gate. B1 requires `completed === true` and not merely a run id — upstream returns `completed: false` when the agent expects further work, which is a live possibility here because `max_turns` is deliberately 6 against an upstream default of 12, and a truncated conversation is not the gate.
...
**Tool-profile closure.** A closed allowlist is not only a privilege boundary — it is the model's entire world. A recommendation tool whose supplier is missing stays advertised and fails at call time on an input the model cannot obtain, with nothing failing loudly: registration succeeds and the lane is quietly dead. `get-theme-styles` was omitted in the first cut of this adapter for exactly that reason, leaving the style lane able only to fabricate a scope or fail with `missing_style_scope`. `AgentsApiToolClosureTest` now classifies every required input of every declared tool as supplier / model-suppliable / explicitly unresolved, and fails on anything new — the same property `MCPServerDiscoveryClosureTest` asserts for the dedicated MCP server. Two inputs remain genuinely unresolved. `recommend-post-blocks` needs a `postId` and Flavor Agent registers no post-listing ability. `recommend-navigation` needs a `wp_navigation` post id or navigation block markup, and nothing in the profile enumerates navigation menus — `list-template-parts` returns `wp_template_part` records, and yields usable markup only in the coincidental case where a template part embeds a Navigation block and the caller holds `edit_theme_options`. Both lanes still work for a caller that already holds the input; neither can be bootstrapped from the profile alone.
...
The bound that finally closes this properly comes from core rather than from the Agents API. `wp_ability_invoked` fires at the top of `WP_Ability::execute()` — core documents it as firing "for every call regardless of outcome (validation failure, permission denial, short-circuit, or successful execution)", ahead of input normalization. `RunContext::bind_invocation()` hooks it and hands the capture to that execution, or drops it if the execution is not the bound ability. A capture therefore cannot outlive the next execution attempt, which is what path-by-path fixes could not guarantee: a call rejected *after* mediation but *before* the callback — by `permission_callback`, by schema validation — leaves nothing behind.

One honest caveat: that action is `@since 7.1.0` and this plugin supports WordPress 7.0+. On 7.0 it never fires and `consume()` falls back to matching the bound ability name, which cannot detect a pre-callback rejection. The residual window there — same process, same ability, after a mediated-but-rejected call — is real on 7.0 and closed on 7.1+.

Two further bounds back it up. The context is bound to the ability the runtime said it was about to dispatch, resolved with upstream's own order (`tool_declaration.ability` → `ability_name` → `metadata.ability_name` → `tool_name`, since a declaration may map a model-facing name onto a different registered ability); and `RunContext::consume()` clears it on read, so one pre-call hook yields at most one attribution. Consumption happens at ability entry in `RecommendationAbilityExecution::execute()`, not inside the request-meta builders — `execute()` returns early for signature-only resolution and for non-array results, and those paths never reach the builders, so consuming there would leave the context set to attribute a later call of the same ability. Both halves are pinned against real upstream source by `test_tool_call_ability_resolution_order_still_matches_upstream` and `test_mediation_context_still_carries_the_keys_run_context_reads` — a drift there would otherwise bind attribution to a name that never executes and stop correlating in silence.
