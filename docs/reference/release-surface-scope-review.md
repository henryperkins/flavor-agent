...
- **Authority** — `StylePrompt::validate_suggestions()` remains the primary place that downgrades a suggestion from executable to advisory for contrast reasons; suggestions should reach the editor with their final tone already set. The editor apply path also runs a minimal parity guard in `src/utils/style-operations.js` before `editEntityRecord()` so persisted, stale, imported, or otherwise unexpected executable entries cannot bypass the same readable-color constraints.
...
- **Out of scope (v1)** — Theme-authored variation selection by itself, custom colors outside the palette, gradients, and duotone. Mixing `set_theme_variation` with readable foreground/background overrides is intentionally unavailable because the variation baseline and manual color override must be reviewed separately.
- **Implementation split** — PHP `StyleContrastValidator` owns response-time downgrade behavior and explanatory `Contrast check:` / `Contrast check unavailable:` messages. JS mirrors only the apply-time pieces needed for all-or-nothing mutation safety: preset/hex resolution, WCAG ratio checks for readable pairs, unsupported/unresolved readable paths, and the theme-variation/readable-color mixed-operation block. Keep the user-facing unavailable message aligned between PHP and JS.
...
- `docs/reference/abilities-and-routes.md` keeps helper/read abilities, preview abilities, recommendation abilities, and the six governed external-apply abilities separated by capability, annotation, and exposure contract.
