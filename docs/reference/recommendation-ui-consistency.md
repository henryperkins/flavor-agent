...
- The review-first recommendation surfaces share `Review first` and `Manual ideas`; the block main panel also uses `Apply now`, `Settings suggestions`, and `Style suggestions` so executable block-level, settings, and style groups remain visibly distinct.
