...
- Content exposes inline read-only `Recent Content Requests` only while its supported/configured panel is mounted, and it has no undo affordance because no Flavor Agent-owned apply occurs.
