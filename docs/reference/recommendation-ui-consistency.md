...
2. prompt composer
3. status notice
4. featured result or collapsed rationale
5. recommendation lane
6. advisory disclosure where the surface still uses one
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| Block                  | `Prompt.php` allows executable local block mutations plus validator-approved selected-block pattern operations behind the structural flag                                             | Yes                                  |
...
| Pattern                | Ranking and inserter shelf/badge rendering, not a review/apply prompt flow                                                                                                            | No lane split                        |
...
`NavigationRecommendations.js` contains both the embedded block subsection and a standalone fallback branch, but the current plugin mounts only the embedded subsection from the block panel. The matrix marks shared pieces as partial for navigation because the mounted nested flow intentionally diverges from full-panel shells. Delegated block Inspector sub-panels now use passive `SuggestionChips` mirrors only, so they are summarized as one lightweight row rather than separate executable surfaces.
...
| Content document panel            | No                   | Yes                  | Yes                 | No                | Yes                 | Yes               | No                  | No                | Yes              | Yes                | Latest Recommendation Workspace; generated output uses `RecommendationHero` as the result container, while the composer collapses behind `Refine request` and support material is disclosure-first |
| Navigation embedded subsection    | Partial              | Partial              | No                  | No                | No                  | Yes               | Partial             | Partial           | Yes              | Yes                | Advisory-only nested surface; mounted flow uses lighter custom sections and stale banner, while standalone branches are unmounted fallback code |
| Pattern inserter affordance       | No                   | No                   | No                  | No                | No                  | No                | No                  | No                | No               | Yes                | Injected local shelf plus loading, empty, error, and setup notices; does not register, reorder, or mutate Gutenberg native pattern categories |
| Template                          | Yes                  | Partial              | No                  | Yes               | Yes                 | Yes               | No                  | Yes               | Yes              | Yes                | Compact preview-first shell; `RecommendationHero` is reserved for stale refresh, while fresh results render a collapsed rationale and one recommendations list |
| Template-part                     | Yes                  | No                   | No                  | Yes               | Yes                 | Yes               | No                  | Yes               | Yes              | Yes                | Compact structural surface with collapsed rationale, one recommendations list, and per-card detail disclosures                 |
...
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| Block inspector main panel        | `Apply now` for inline-safe block-level attributes, `Settings suggestions` and `Style suggestions` for executable settings/style groups, and `Review first` for validator-approved structural operations, with `Stale` when stale | `Manual ideas` via `AIAdvisorySection`                                                                                                | Fresh results use lane labels; stale refresh uses the hero tone; advisory section shows `Advisory only`                            | Compact direct local-attribute apply plus reviewed structural apply |
...
| Content                           | None                                                                                                                         | `Editorial Notes` via `AIAdvisorySection`, collapsed by default after a usable result                                                  | Result hero uses only the generated title and mode pill; generated text can be copied manually when fresh                          | Editorial output first, support notes second, no apply lane |
...
| Template                          | `Recommendations` list; executable cards keep the `Review` action                                                            | Advisory cards live in the same list and use a compact `Manual` badge                                                                  | Fresh cards show summary/count pills plus `Ready` or `Manual`; details are collapsed; button uses `Review` / `Reviewing`          | Preview-confirm flow with bounded advisory fallbacks        |
| Template-part                     | `Recommendations` list; executable cards keep the `Review` action                                                            | Advisory cards live in the same list and use a compact `Manual` badge                                                                  | Fresh cards show summary/count pills plus `Ready` or `Manual`; focus blocks and patterns are collapsed behind `Details`           | Compact structural surface with review where applicable     |
...
- The fuller review-first recommendation surfaces share `Review first` and `Manual ideas`, but Template and Template-Part intentionally collapse executable and advisory cards into one `Recommendations` list to stay navigable inside the document sidebar. The block main panel still uses `Apply now`, `Settings suggestions`, and `Style suggestions` so executable block-level, settings, and style groups remain visibly distinct.
- The advisory badge policy is standardized through `AIAdvisorySection` for surfaces that still use that shell; template and template-part intentionally use `Manual` inside their single compact recommendations list.
...
- Content remains editorial-only. It can generate drafts, revision text, critiques, and review notes, but it does not mutate post content or enter preview/apply; copy-to-clipboard is the manual handoff.
...
- The active card stays visibly selected and changes its action to `Reviewing`, while the shared lower panel owns confirm and cancel.
...
| Block inspector passive subpanels | No                                        | No surface-level undo                                                                             | Mirrors the latest result passively; stale projected chips are marked stale and tell the user to refresh the main `AI Recommendations` panel, but delegated sub-panels do not own refresh, apply, or activity state |
| Content                           | Collapsed read-only request history       | No Flavor Agent undo                                                                              | Keeps stale results visible, marks them stale, disables generated-content copy, offers refresh                                           |
...
| Pattern inserter                  | No inline history; scoped audit rows only | No Flavor Agent undo                                                                              | Insert-time server signature mismatch blocks insertion and refreshes recommendations; no persistent stale panel state                     |
| Template                          | Collapsed latest-valid apply history      | Shared latest-valid tail undo; validated undo preparation must still succeed                      | Keeps stale results visible, marks them stale, disables apply, offers refresh                                                             |
| Template-part                     | Collapsed latest-valid apply history      | Shared latest-valid tail undo; validated undo preparation must still succeed                      | Keeps stale results visible, marks them stale, disables apply, offers refresh                                                             |
...
- Content exposes inline read-only `Recent Content Requests` only while its supported/configured panel is mounted, collapsed by default with one visible row before overflow, and it has no undo affordance because no Flavor Agent-owned apply occurs.
- Content now preserves stale generated results with a refresh banner and disabled copy action. This aligns the trust interruption with the other request-owning surfaces while keeping the lane editorial-only.
...
| Navigation                        | Yes             | Yes                             | No          | Partial: embedded stale banner; standalone scope-bar branch is unmounted fallback code |
...
- `Review first`: deterministic change staged for review; preview/confirm is required before apply where an apply path exists
